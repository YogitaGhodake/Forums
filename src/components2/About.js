import React from 'react'
import Footer from './Header/Footer'
import HeaderLight from './Header/HeaderLight'

function About() {
    return (
        <div>
            <HeaderLight />
            <div>
                <section class="hero-area section-padding bg-gray">
                    <span class="icon-shape icon-shape-1 is-scale"></span>
                    <span class="icon-shape icon-shape-2 is-bounce"></span>
                    <span class="icon-shape icon-shape-3 is-swing"></span>
                    <span class="icon-shape icon-shape-4 is-spin"></span>
                    <span class="icon-shape icon-shape-5 is-spin"></span>
                    <span class="icon-shape icon-shape-6 is-bounce"></span>
                    <span class="icon-shape icon-shape-7 is-tilt"></span>
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="hero-content">
                                    <h3 class="fs-16 fw-medium pb-3">Who we are</h3>
                                    <h2 class="section-title fs-40 pb-3 lh-55">Helping developers and technologists write the script of the future.</h2>
                                    <p class="lh-26 pb-3">Our public platform <strong class="fw-medium text-black">serves 100 million people every month</strong>, making it one of the 50 most popular websites in the world.</p>
                                    <p class="lh-26">Our asynchronous knowledge management and collaboration offering <strong class="fw-medium text-black">Disilab for Teams</strong>, is transforming how people work.</p>
                                </div>
                                {/* <!-- end hero-content --> */}
                            </div>
                            {/* <!-- end col-lg-6 --> */}
                            <div class="col-lg-6">
                                <div class="generic-img-box h-100">
                                    <img class="lazy" src="/assets/images/img-loading.png" data-src="/assets/images/img1.jpg" alt="image" />
                                    <img class="lazy" src="assets/images/img-loading.png" data-src="assets/images/img2.jpg" alt="image" />
                                    <img class="lazy" src="assets/images/img-loading.png" data-src="assets/images/img3.jpg" alt="image" />
                                    <img class="lazy" src="assets/images/img-loading.png" data-src="assets/images/img4.jpg" alt="image" />
                                </div>
                            </div>
                            {/* <!-- end col-lg-6 --> */}
                        </div>
                        {/* <!-- end row --> */}
                    </div>
                    {/* <!-- end container --> */}
                </section>

                {/* <!-- ================================
         START FUNFACT AREA
================================= --> */}
                <section class="funfact-area">
                    <div class="container">
                        <div class="counter-box bg-white shadow-md rounded-rounded px-4">
                            <div class="row">
                                <div class="col responsive-column-half border-right border-right-gray">
                                    <div class="media media-card text-center px-0 py-4 shadow-none rounded-0 bg-transparent counter-item mb-0">
                                        <div class="media-body">
                                            <h5 class="fw-semi-bold pb-2">80+ million</h5>
                                            <p class="lh-20">Monthly visitors to our network</p>
                                        </div>
                                    </div>
                                </div>
                                {/* <!-- end col --> */}
                                <div class="col responsive-column-half border-right border-right-gray">
                                    <div class="media media-card text-center px-0 py-4 shadow-none rounded-0 bg-transparent counter-item mb-0">
                                        <div class="media-body">
                                            <h5 class="fw-semi-bold pb-2">25+ Million</h5>
                                            <p class="lh-20">Questions asked to-date</p>
                                        </div>
                                    </div>
                                </div>
                                {/* <!-- end col --> */}
                                <div class="col responsive-column-half border-right border-right-gray">
                                    <div class="media media-card text-center px-0 py-4 shadow-none rounded-0 bg-transparent counter-item mb-0">
                                        <div class="media-body">
                                            <h5 class="fw-semi-bold pb-2">14.7 seconds</h5>
                                            <p class="lh-20">Average time between new questions</p>
                                        </div>
                                    </div>
                                </div>
                                {/* <!-- end col --> */}
                                <div class="col responsive-column-half border-right border-right-gray">
                                    <div class="media media-card text-center px-0 py-4 shadow-none rounded-0 bg-transparent counter-item mb-0">
                                        <div class="media-body">
                                            <h5 class="fw-semi-bold pb-2">40+ Billion</h5>
                                            <p class="lh-20">Times a developer got help</p>
                                        </div>
                                    </div>
                                </div>
                                {/* <!-- end col --> */}
                                <div class="col responsive-column-half">
                                    <div class="media media-card text-center px-0 py-4 shadow-none rounded-0 bg-transparent counter-item mb-0">
                                        <div class="media-body">
                                            <h5 class="fw-semi-bold pb-2">10,000+</h5>
                                            <p class="lh-20">Customer companies for all products</p>
                                        </div>
                                    </div>
                                </div>
                                {/* <!-- end col --> */}
                            </div>
                            {/* <!-- end row --> */}
                        </div>
                        {/* <!-- end counter-box --> */}
                    </div>
                    {/* <!-- end container --> */}
                </section>
                {/* <!-- end funfact-area --> */}




                {/* <!-- ================================
         START AWARD AREA
================================= --> */}
                <section class="award-area pt-120px pb-70px">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-4 responsive-column-half">
                                <div class="card card-item hover-y text-center award-card">
                                    <a href="#" class="card-body">
                                    <svg className="svg-icon mb-3" width={96} height={52} fill="none">
                                    <path
                                    fillRule="evenodd"
                                    clipRule="evenodd"
                                    d="M23.456 30.668c0 2.686-6.703 3.693-7.08 3.748.148.022 1.02.16 2.111.409 3.248.629 5.388 1.722 5.388 3.001v10.179l-.039.08c-.65 1.823-5.721 3.248-11.882 3.248S.715 49.908.072 48.085H0V37.9c0-.633.529-1.228 1.46-1.747-.71-.7-.456-2.246.3-2.681C2.998 33.392 18 32.144 18 30.736c0-1.005-8.192-1.307-13.04-1.486-1.97-.072-3.388-.124-3.48-.195-.842-.639-.6-1.818.192-2.28C2.508 26.724 18 25.595 18 24.188c0-1.008-8.308-1.575-13.193-1.91-1.966-.133-3.378-.23-3.47-.302-.84-.638-.604-1.818.193-2.28l.058-.003c1.225-.075 16.544-1.01 16.544-2.362 0-.828-5.852-1.182-10.68-1.474-3.17-.191-5.9-.356-6.235-.611-.842-.639-.6-1.819.192-2.28C3.07 12.863 18 11.894 18 10.486c0-1.02-8.415-1.625-13.26-1.973h-.001c-1.892-.136-3.24-.233-3.33-.3-.45-.342-.066-2.132.193-2.28l.143-.01C3.372 5.808 18.22 4.759 18.22 3.716c0-1.1-6.76-1.738-6.76-1.738L12.999 0s9.88.712 9.88 3.79c0 2.365-7.476 3.317-7.476 3.317s7.542 1.322 7.542 3.391c0 2.69-7.157 3.465-7.157 3.465s7.371 1.123 7.371 3.232c0 2.479-7.949 3.619-7.949 3.619s8.142 1.345 8.142 3.157c0 2.285-7.646 3.596-7.646 3.596s7.75.627 7.75 3.1zm16.416 8.001h-.688l-4.434 9.934h.737l1.21-2.758h5.628l1.21 2.758h.77l-4.433-9.934zm-.347.798 2.503 5.711h-5.022l2.52-5.71zm5.287-.73h-.787l3.45 9.94h.577l2.789-8.571 2.789 8.571h.572l3.455-9.94h-.738l-2.992 8.783-2.784-8.81h-.55l-2.789 8.805-2.992-8.777zm17.675-.068h-.683l-4.434 9.934h.738l1.21-2.758h5.622l1.21 2.758h.77l-4.434-9.934zm-.353.798 2.503 5.711h-5.022l2.52-5.71zm10.194-.73h-4v9.866h.705v-4.086h3.119l3.03 4.092h.897l-3.14-4.218c1.606-.227 2.789-1.168 2.789-2.82v-.03a2.654 2.654 0 0 0-.721-1.834c-.572-.587-1.513-.97-2.68-.97zm2.69 2.862c0 1.425-1.177 2.257-2.773 2.257h-3.207v-4.44h3.246c1.727 0 2.734.826 2.734 2.154v.029zm5.98-2.861h-3.175v9.865h3.175c2.992 0 5.06-2.154 5.06-4.935v-.029c0-2.775-2.068-4.901-5.06-4.901zm4.323 4.964c0 2.348-1.672 4.229-4.323 4.229v-.006h-2.465v-8.509h2.465c2.651 0 4.323 1.915 4.323 4.257v.029zm2.52 2.912-.462.553c1.144 1.06 2.31 1.579 3.84 1.579 1.831 0 3.152-1.095 3.152-2.69v-.029c0-1.442-.925-2.257-3.125-2.724-2.261-.479-2.778-1.111-2.778-2.154v-.029c0-1.031.924-1.863 2.327-1.863 1.051-.02 2.07.38 2.844 1.117l.446-.582c-.963-.798-1.876-1.197-3.263-1.197-1.771 0-3.064 1.129-3.064 2.593v.029c0 1.493.924 2.297 3.213 2.776 2.162.45 2.695 1.06 2.695 2.103v.028c0 1.128-.984 1.96-2.415 1.96-1.397 0-2.365-.467-3.41-1.47zM37.187 8.897h-3.322v-.678h7.349v.678H37.89v9.193h-.704V8.897zm5.562-.678h.71v4.57h6.094V8.22h.71v9.87h-.71v-4.627h-6.095v4.628h-.71V8.219zm10.006 0h6.772v.678h-6.068v3.875h5.457v.678H53.46v3.962h6.14v.678h-6.844V8.219zm-8.73 14.8v.006h-2.426l-.902 3.516-.003.01c-.57 2.248-.68 2.684-1.086 4.937-.325-2.468-.451-3.06-1.634-8.463H35.24l2.783 11.381h2.828l1.056-3.835c.382-1.21.694-2.44.935-3.688.3 1.45.355 1.644.803 3.234l.149.528c.216.755.42 1.515.625 2.275.133.495.267.991.404 1.486h2.827l2.784-11.387H47.71l-.715 3.471c-.496 2.314-.573 2.702-.903 4.958-.326-2.107-.44-2.523-.978-4.47l-.078-.283c-.352-1.225-.682-2.45-1.012-3.676zm16.443 9.94c.544-.525.907-1.066 1.276-1.636l-2.35-1.237c-.384.775-.967 1.927-2.673 1.927-1.677 0-2.426-1.357-2.41-2.725h7.554c-.044-1.236-.127-3.374-1.502-4.907-1.353-1.51-3.174-1.573-3.801-1.573-4.17 0-5.424 3.397-5.424 6.042 0 3.584 2.206 5.659 5.341 5.659a5.694 5.694 0 0 0 3.989-1.55zm-2.41-7.233c.355.436.562.98.589 1.55h-4.313l.003-.012c.098-.459.204-.955.624-1.435a2.162 2.162 0 0 1 1.528-.714c.58-.02 1.145.2 1.57.611zM68.07 23.02h-4.247v11.382h4.638c1.26 0 2.145-.052 2.97-.422.53-.224.985-.604 1.308-1.093.322-.49.499-1.067.507-1.66 0-.65-.214-1.276-.605-1.71-.597-.703-1.468-.85-2.136-.964l-.025-.005c.594-.125 1.65-.353 2.145-1.515.05-.109.203-.48.203-1.095a2.744 2.744 0 0 0-.51-1.66 2.585 2.585 0 0 0-1.388-.984c-.665-.223-1.705-.274-2.86-.274zm-1.634 4.497V25.15h2.256c.528 0 1.446.045 1.446 1.14 0 1.18-1.012 1.208-1.633 1.225h-2.069zm0 4.822V29.58h2.396c.566.018 1.631.05 1.631 1.402 0 1.305-1.194 1.34-2.112 1.356h-1.915zm13.302-9.319H75.49v11.382h4.637c1.26 0 2.146-.052 2.97-.422a3.036 3.036 0 0 0 1.816-2.753c0-.65-.22-1.276-.605-1.71-.597-.703-1.468-.85-2.136-.964l-.026-.005c.594-.125 1.65-.353 2.146-1.515.05-.109.203-.48.203-1.095a2.744 2.744 0 0 0-.51-1.66 2.585 2.585 0 0 0-1.388-.984c-.67-.223-1.71-.274-2.86-.274zm-1.634 4.497V25.15h2.255c.528 0 1.447.045 1.447 1.14 0 1.18-1.012 1.208-1.634 1.225h-2.068zm0 4.822V29.58H80.5c.566.018 1.63.05 1.63 1.402 0 1.305-1.198 1.34-2.112 1.356h-1.914zm9.907-9.319h-3.163l4.126 6.081v5.3h2.706v-5.385l4.154-5.996h-3.125l-1.881 3.05c-.286.467-.314.53-.451.854L88.01 23.02z"
                                    fill="#0d233e"
                                    />
                                </svg>
                                        <p class="lh-24">In May we were named the <span class="fw-medium">People’s Voice Winner</span> in the Webby Awards</p>
                                    </a>
                                </div>
                                {/* <!-- end card --> */}
                            </div>
                            {/* <!-- end col-lg-4 --> */}
                            <div class="col-lg-4 responsive-column-half">
                                <div class="card card-item hover-y text-center award-card">
                                    <a href="#" class="card-body">
                                    <svg className="mb-3" width={50} height={55} fill="none">
                                        <path d="m21.795 46 21.569-12.025V.57H0v33.181L21.795 46z" fill="#F74222" />
                                        <path
                                        d="M24.52 10.479c-.909-.223-1.817-.223-2.725-.223H20.66c-7.265.223-12.714 6.236-12.487 13.362.227 7.126 6.357 12.47 13.622 12.247 2.952 0 5.676-.89 7.946-2.45l-2.95-4.898a8.029 8.029 0 0 1-4.996 1.781h-.908c-4.087-.222-7.265-3.785-7.038-7.794.227-4.008 3.86-7.126 7.946-6.903l2.725-5.122z"
                                        fill="#fff"
                                        />
                                        <path
                                        d="m34.51 26.735-3.406-5.567h-6.357l-3.406 5.567h6.584l3.179 5.345 3.405-5.345zm-8.174-10.02c-.454.667-.681 1.558-.681 2.449l6.13.223v-1.56h-4.087c.227-.667.681-.89 1.362-1.335l.681-.446c1.363-.445 2.044-1.336 2.044-2.45 0-.667-.454-1.335-.908-1.78-.454-.446-1.363-.669-2.044-.669-.68 0-1.362 0-2.043.445-.454.446-.908.891-1.135 1.336l1.135 1.114c.454-.89 1.135-1.336 2.043-1.336.455 0 1.136.445 1.136.89 0 .446-.228.669-1.136 1.114l-.454.223c-.68.445-1.589 1.113-2.043 1.781z"
                                        fill="#fff"
                                        />
                                    </svg>
                                        <p class="lh-24"><span class="fw-medium">We’re a seasoned winner in the G2 awards</span> Spring 2019 to Winter 2021</p>
                                    </a>
                                </div>
                                {/* <!-- end card --> */}
                            </div>
                            {/* <!-- end col-lg-4 --> */}
                            <div class="col-lg-4 responsive-column-half">
                                <div class="card card-item hover-y text-center award-card">
                                    <a href="#" class="card-body">
                                        <svg class="mb-3" fill="none" height="55" viewBox="0 0 259 391" width="80" xmlns="http://www.w3.org/2000/svg"><path d="m237.649 87.7771h-216.291c-5.6595.0158-11.0827 2.2614-15.08459 6.2461-4.00191 3.9847-6.2572159 9.3848-6.27316586 15.0198v171.477c0 11.716 7.71700586 26.963 17.14665586 33.932l95.2031 70.324c9.437 6.968 24.85 6.968 34.307 0l95.203-70.324c9.403-6.969 17.14-22.242 17.14-33.932v-171.477c-.016-5.634-2.27-11.033-6.271-15.0174-4-3.9845-9.421-6.2309-15.08-6.2485z" fill="#fff" /><path d="m182.911 317.354-53.414 30.02-53.408-30.02z" fill="#27aae1" /><g fill="#231f20"><path d="m237.649 102.944c1.624.002 3.181.645 4.329 1.788 1.149 1.144 1.795 2.694 1.796 4.311v171.477c0 6.881-5.453 17.655-11.001 21.747l-95.196 70.331c-1.874 1.385-4.97 2.241-8.06 2.241-3.089 0-6.199-.856-8.059-2.241l-95.2642-70.331c-5.5544-4.092-10.9946-14.866-10.9946-21.747v-171.477c.0018-1.618.6486-3.17 1.7984-4.313 1.1498-1.144 2.7084-1.786 4.3336-1.786zm0-4.0124h-216.291c-2.6998.0017-5.2886 1.0704-7.1977 2.9714s-2.9824 4.478-2.9842 7.167v171.45c0 8.199 6.0447 20.109 12.6267 24.997l95.2032 70.311c2.579 1.9 6.501 3.036 10.491 3.036 3.989 0 7.925-1.136 10.491-3.036l95.203-70.311c6.595-4.888 12.64-16.798 12.64-24.997v-171.477c-.003-2.688-1.077-5.264-2.986-7.165-1.909-1.9004-4.496-2.9697-7.196-2.9732z" /><path d="m48.8612 141.102c.2104-1.502.4164-3.007.6179-4.514.2821-2.046-.6716-3.029-2.841-2.929l-1.1686.054c-2.1291.093-3.3582 1.237-3.6403 3.41-.4074 3.009-.8193 6.019-1.2358 9.028-.1343 1.01 0 1.658.3493 1.946.5833.409 1.2093.755 1.8671 1.03 2.2388.838 4.4775 1.625 6.7163 2.361 1.0703.357 2.1177.779 3.1365 1.263.5679.289 1.0358.741 1.3433 1.298.2888.548.3224 1.678.0873 3.404l-1.397 10.398c-.7254 5.464-4.3051 9.149-10.6319 11.463-1.6074.584-3.2081 1.183-4.8021 1.798-6.226 2.461-8.8051.321-7.8514-6.807.1276-.963.262-1.926.3896-2.889 2.5432-.847 5.0999-1.661 7.67-2.441-.1411.983-.2754 2.006-.4097 2.956-.2888 2.126.7119 2.782 3.0357 2.006l1.3433-.455c.9217-.238 1.7492-.749 2.3728-1.465.6237-.717 1.0141-1.605 1.1197-2.547.4029-2.916.8037-5.832 1.2022-8.747.1276-.95 0-1.545-.3694-1.792-.608-.335-1.2439-.617-1.9007-.843-2.2567-.669-4.4865-1.451-6.7163-2.254-1.1069-.32-2.1662-.786-3.1499-1.384-.6413-.479-1.0681-1.19-1.1888-1.979-.1966-1.181-.1966-2.385 0-3.565l1.5917-11.79c.8955-6.633 4.3052-9.924 10.2289-9.87h4.641c5.9237.04 8.5118 2.826 7.764 8.359l-.6716 4.755c-2.4895.232-4.9902.479-7.5021.742z" /><path d="m75.5785 132.515c-1.5313 12.733-3.0648 25.464-4.6006 38.192-2.6865.622-5.373 1.297-8.0596 2.006 1.7105-13.312 3.4209-26.629 5.1313-39.95-2.391.08-4.7887.167-7.1797.261.2552-1.926.5171-3.846.7723-5.771h21.6601c-.1881 1.685-.3762 3.377-.5575 5.062-2.3776.062-4.7663.129-7.1663.2z" /><path d="m87.5805 127.253h14.5475c5.664 0 8.389 2.146 8.174 6.439-.161 3.09-.32 6.177-.477 9.262-.168 3.284-1.827 5.411-4.997 6.487 1.495 5.163 3.063 10.272 4.701 15.328-2.588.196-5.176.432-7.764.708-1.437-4.955-2.8138-9.964-4.1168-15.059-1.585.124-3.1678.26-4.7484.407-.4791 5.324-.9537 10.647-1.4239 15.97-2.7133.401-5.4245.847-8.1334 1.337 1.4194-13.624 2.8321-27.251 4.238-40.879zm5.7357 19.058c1.7015-.124 3.4007-.245 5.0977-.361 2.3301-.147 3.5601-1.136 3.6871-2.989l.584-8.279c.13-1.854-.926-2.764-3.1697-2.728l-4.9297.087z" /><path d="m123.311 164.086c-2.731.058-5.467.163-8.207.315l1.108-30.762c.161-4.253 3.201-6.375 9.121-6.366h5.568c5.923 0 8.955 2.115 9.093 6.346l.947 30.702c-2.731-.125-5.469-.21-8.214-.255-.04-4.493-.082-8.985-.127-13.475-3.029 0-6.045 0-9.081 0-.067 4.499-.136 8.997-.208 13.495zm9.248-17.975-.12-12.245c0-1.437-1.122-2.16-3.312-2.16h-2.014c-2.194 0-3.303.723-3.325 2.167l-.208 12.258c2.995-.031 5.989-.038 8.979-.02z" /><path d="m158.578 131.954c.896 11.368 1.791 22.737 2.687 34.105-2.722-.347-5.449-.651-8.181-.909-.671-11.101-1.343-22.202-2.014-33.303l-7.214-.107c-.067-1.503-.13-3.007-.188-4.514h21.66c.152 1.609.302 3.221.45 4.835z" /><path d="m193.718 127.253 4.997 37.917c.786 5.965-2.015 7.924-8.496 6.279-2.239-.553-4.505-1.079-6.797-1.578-6.461-1.418-9.974-4.514-10.491-9.683-1.115-10.972-2.234-21.95-3.358-32.935h7.428c1.312 11.462 2.619 22.924 3.922 34.386.209 1.846 1.478 2.976 3.809 3.444l3.237.669c2.162.468 3.116-.254 2.874-2.207l-4.553-36.292z" /><path d="m216.936 141.884c-.215-1.591-.436-3.183-.672-4.774-.299-2.18-1.538-3.322-3.714-3.424l-1.168-.054c-2.129-.093-3.049.89-2.767 2.936l1.168 8.546c.044.755.362 1.468.894 2.007.47.394 1.215.923 2.236 1.611 2.566 1.565 5.131 3.21 7.697 4.936 1.263.853 2.478 1.775 3.64 2.761.708.628 1.311 1.364 1.787 2.181.613 1.348.988 2.791 1.108 4.266.551 4.093 1.104 8.185 1.659 12.278.873 6.54-1.773 8.373-8.06 5.892-1.598-.616-3.199-1.213-4.802-1.793-6.273-2.293-9.859-6.226-10.652-12.177-.114-.816-.222-1.632-.336-2.448 2.606.642 5.201 1.311 7.784 2.006l.383 2.796c.269 2.006 1.572 3.343 3.896 4.126l1.377.455c2.048.668 2.921 0 2.632-2.14l-1.269-9.249c-.054-.813-.375-1.587-.913-2.2-.716-.656-1.47-1.27-2.257-1.839-2.593-1.81-5.181-3.536-7.764-5.176-1.288-.758-2.532-1.586-3.728-2.481-.757-.588-1.376-1.335-1.813-2.187-.476-.997-.78-2.066-.9-3.163l-1.343-9.991c-.744-5.528 1.844-8.315 7.764-8.359h4.641c5.923-.045 9.333 3.245 10.229 9.87.25 1.873.503 3.745.758 5.618-2.502-.281-5.001-.558-7.495-.83z" /><path d="m52.9111 293.099h-8.0595c1.7686-15.907 3.5372-31.818 5.3058-47.734.7455-6.648 3.9291-8.64 9.6715-6.414 1.8067.669 3.6133 1.338 5.4334 1.946 5.8029 2.007 8.5364 5.604 8.0999 11.235l-3.2037 40.967h-8.0595c.5821-6.313 1.1507-12.619 1.7059-18.919-2.9686-.334-5.9305-.697-8.8856-1.09zm11.458-25.211c.5059-5.738 1.0141-11.476 1.5246-17.214.1746-2.006-.8127-3.343-2.9552-4.012l-2.0149-.615c-2.1358-.669-3.3044 0-3.5193 2.16l-1.8336 18.209c2.9328.535 5.8656 1.026 8.7984 1.472z" /><path d="m119.073 251.122c-2.968 13.863-6.112 27.793-9.403 41.977h-8.731c-1.343-8.39-2.6169-16.912-3.8213-25.566-1.9343 8.386-3.9268 16.908-5.9775 25.566h-8.7312c-2.0149-15.515-3.7813-31.645-5.3059-48.577 2.4403.664 4.885 1.28 7.3342 1.846 1.0209 12.077 2.1403 23.845 3.3582 35.302 2.4984-11.301 4.8984-22.358 7.1998-33.169 1.9522.334 3.9089.64 5.8697.916 1.344 11.146 2.783 22.113 4.319 32.902 2.207-10.7 4.33-21.264 6.367-31.691 2.516.214 5.024.379 7.522.494z" /><path d="m129.927 293.099h-8.06c.094-11.56.193-23.123.296-34.687.047-4.795 3.089-7.022 9.141-7.109 1.898-.027 3.794-.085 5.688-.174 6.045-.308 9.128 1.772 9.262 6.687.3 11.77.607 23.538.92 35.303h-8.059c-.072-5.131-.148-10.258-.229-15.381-3.009.063-6.018.096-9.026.1zm8.906-20.497c-.068-4.659-.137-9.324-.209-13.996 0-1.639-1.155-2.401-3.358-2.328l-2.062.054c-2.23.053-3.358.869-3.358 2.501 0 4.637.011 9.271.034 13.903 2.964-.013 5.948-.058 8.953-.134z" /><path d="m152.762 249.758c4.925-.628 9.824-1.449 14.695-2.461 5.689-1.217 8.698.722 9.087 6.219.269 3.928.54 7.854.813 11.777.289 4.18-1.054 7.002-4.07 8.359 2.239 6.322 4.518 12.804 6.837 19.447h-7.63c-2.135-6.242-4.22-12.336-6.252-18.283-1.572.113-3.15.227-4.702.327.278 5.988.558 11.973.84 17.956h-8.06c-.519-14.445-1.039-28.892-1.558-43.341zm8.523 20.31c1.693-.141 3.392-.295 5.084-.462 2.317-.22 3.405-1.464 3.278-3.684l-.585-9.938c-.127-2.227-1.343-3.123-3.573-2.735-1.647.281-3.299.542-4.956.782.251 5.346.501 10.691.752 16.037z" /><path d="m185.987 293.099c-1.289-16.495-2.576-32.975-3.861-49.44 4.262-1.23 8.491-2.614 12.687-4.152 6.756-2.562 10.47-.408 11.31 7.061 1.419 12.729 2.834 25.459 4.245 38.192.631 5.604-2.279 8.384-8.732 8.339zm7.489-6.266 5.964-.227c2.096-.081 3.063-.943 2.902-2.568-1.174-11.685-2.351-23.366-3.533-35.042-.269-2.675-1.491-3.598-3.674-2.902-1.706.541-3.419 1.056-5.131 1.558 1.155 13.062 2.312 26.123 3.472 39.181z" /></g><path d="m31.869 209.721h44.22" stroke="#27aae1" stroke-linecap="round" stroke-miterlimit="10" stroke-width="7.2" /><path d="m259 102.944c0-15.5886-19.477-33.9988-38.168-35.5838-1.836-10.1147-7.058-19.3121-14.817-26.0923-7.758-6.7802-17.595-10.7433-27.906-11.2432-.026-2.0672-.627-4.0868-1.735-5.8354-1.108-1.7485-2.68-3.1579-4.542-4.0722s-3.942-1.298-6.01-1.1085c-2.067.1895-4.042.9447-5.706 2.1821-5.748-7.214-13.195-12.90491-21.68-16.5686-8.485-3.6637-17.747-5.187054-26.965-4.435108-9.218.751947-18.107 3.755978-25.8802 8.745968-7.7732 4.98994-14.1903 11.81184-18.6829 19.86134-4.7092-1.5868-9.7226-2.0704-14.6502-1.4132-4.9275.6572-9.6364 2.4374-13.7602 5.2022s-7.5515 6.4396-10.0163 10.7385-3.9003 9.1061-4.1949 14.0475c-7.4102 4.2771-13.557 10.4238-17.82081 17.8204-4.26378 7.3966-6.4937841 15.7817-6.46521899 24.31v181.0203c0 11.716 7.71700899 26.963 17.14662899 33.931l95.2031 70.325c9.437 6.968 24.85 6.968 34.307 0l95.203-70.325c9.403-6.968 17.14-22.242 17.14-33.931zm-6.716 177.576c0 9.536-6.75 22.884-14.44 28.568l-95.183 70.331c-3.358 2.481-8.16 3.906-13.157 3.906s-9.793-1.425-13.151-3.906l-95.2099-70.331c-7.7103-5.684-14.42655-19.032-14.42655-28.568v-171.477c.00532-3.866 1.54919-7.573 4.29345-10.3078 2.7443-2.735 6.4653-4.2756 10.348-4.2845h216.264c3.886.0036 7.611 1.5421 10.359 4.2778 2.748 2.7355 4.293 6.4455 4.296 10.3145z" fill="#27aae1" /><path d="m180.749 209.721h44.22" stroke="#27aae1" stroke-linecap="round" stroke-miterlimit="10" stroke-width="7.2" /><path d="m85.2097 215.064c3.5059-2.608 11.4177-6.687 11.4177-11.368 0-1.144-.4903-2.956-3.4589-2.956-1.0696.007-2.1261.235-3.1029.669.1245.294.1863.61.1813.929 0 .714-.2848 1.399-.7917 1.903-.507.505-1.1946.789-1.9116.789-.7169 0-1.4045-.284-1.9115-.789-.507-.504-.7918-1.189-.7918-1.903 0-3.002 4.0768-5.35 8.9528-5.35 4.2985 0 7.8849 1.813 7.8849 6.226 0 4.721-4.117 8.025-9.2617 11.034h9.9737v4.233h-16.381z" fill="#231f20" /><path d="m105.647 207.976c0-5.919 2.922-10.994 10.075-10.994s10.074 5.075 10.074 10.994c0 5.918-2.921 10.987-10.074 10.987s-10.075-5.049-10.075-10.987zm15.025 0c0-4.374-1.464-7.196-4.917-7.196-3.452 0-4.936 2.849-4.936 7.196 0 4.346 1.464 7.188 4.923 7.188s4.93-2.795 4.93-7.188z" fill="#231f20" /><path d="m129.255 215.064c3.499-2.608 11.418-6.687 11.418-11.368 0-1.144-.484-2.956-3.452-2.956-1.07.006-2.127.234-3.103.669.119.295.178.611.174.929 0 .714-.285 1.399-.792 1.903-.507.505-1.194.789-1.911.789s-1.405-.284-1.912-.789c-.507-.504-.791-1.189-.791-1.903 0-3.002 4.076-5.35 8.952-5.35 4.299 0 7.892 1.813 7.892 6.226 0 4.721-4.124 8.025-9.268 11.034h9.973v4.233h-16.387z" fill="#231f20" /><path d="m149.686 207.976c0-5.919 2.928-10.994 10.074-10.994 7.147 0 10.075 5.075 10.075 10.994 0 5.918-2.922 10.987-10.075 10.987-7.152 0-10.074-5.049-10.074-10.987zm15.024 0c0-4.374-1.457-7.196-4.916-7.196s-4.923 2.822-4.923 7.196c0 4.373 1.464 7.188 4.923 7.188s4.916-2.795 4.916-7.188z" fill="#231f20" /></svg>
                                        <p class="lh-24">Disilab for Teams won the <span class="fw-medium">2020 Stratus Award for Cloud Collaboration</span></p>
                                    </a>
                                </div>
                                {/* <!-- end card --> */}
                            </div>
                            {/* <!-- end col-lg-4 --> */}
                            <div class="col-lg-10 mx-auto">
                                <div class="award-content text-center">
                                    <p class="section-desc pb-3">Founded in 2009, Disilab’s public platform is used by nearly everyone who codes to learn, share their knowledge, collaborate, and build their careers. Our products and tools help developers and technologists in life and at work. These products include
                                        <a href="#" class="text-color hover-underline">Disilab for Teams</a>, <a href="#" class="text-color hover-underline">Disilab Advertising</a>, and Disilab for
                                        <a href="#" class="text-color hover-underline">Talent</a> and <a href="#" class="text-color hover-underline">Jobs</a>.</p>
                                    <p class="section-desc pb-3">Disilab for Teams, our core SaaS collaboration product, is helping thousands of companies around the world as the transition to remote work, address business continuity challenges, and undergo digital transformation.</p>
                                    <p class="section-desc pb-3">Whether it’s on Disilab or within Disilab for Teams, community is at the center of all that we do.</p>
                                </div>
                            </div>
                        </div>
                        {/* <!-- end row--> */}
                    </div>
                    {/* <!-- end container --> */}
                </section>
                {/* <!-- end award-area --> */}




                {/* <!-- ================================
         START ABOUT AREA
================================= --> */}
                <section class="about-area pb-120px">
                    <div class="container">
                        <div class="about-box bg-white shadow-md rounded-rounded px-5 pt-40px pb-90px">
                            <div class="about-content pb-90px text-center">
                                <h2 class="section-title fs-30 pb-3 lh-40">Our mission is to share and grow
                                    <br /> the world’s knowledge.</h2>
                                <p class="section-desc pb-3">Disilab mission is to share and grow the world’s knowledge. A vast amount of the knowledge that would be valuable to many people is currently only available to a few — either locked in people’s heads, or only accessible to select groups.</p>
                                <p class="section-desc pb-3">We want to connect the people who have knowledge to the people who need it, to bring together people with different perspectives so they can understand each other better, and to empower everyone to share their knowledge for the benefit of the rest of the world.</p>
                            </div>
                            {/* <!-- end about-content --> */}
                            <div class="client-logo-box pb-60px">
                                <h4 class="fs-18 pb-5 text-center text-gray">Used by thousands of organizations around the globe</h4>
                                <div class="row justify-content-center text-center">
                                    <div class="client-logo-item col-lg-3 mb-2 responsive-column-half">
                                        <img src="assets/images/client-logo.png" alt="client-logo" class="filter-gray" />
                                    </div>
                                    {/* <!-- end client-logo-item --> */}
                                    <div class="client-logo-item col-lg-3 mb-2 responsive-column-half">
                                        <img src="assets/images/client-logo-2.png" alt="client-logo" class="filter-gray" />
                                    </div>
                                    {/* <!-- end client-logo-item --> */}
                                    <div class="client-logo-item col-lg-3 mb-2 responsive-column-half">
                                        <img src="assets/images/client-logo-3.png" alt="client-logo" class="filter-gray" />
                                    </div>
                                    {/* <!-- end client-logo-item --> */}
                                    <div class="client-logo-item col-lg-3 mb-2 responsive-column-half">
                                        <img src="assets/images/client-logo-4.png" alt="client-logo" class="filter-gray" />
                                    </div>
                                    {/* <!-- end client-logo-item --> */}
                                    <div class="client-logo-item col-lg-3 mb-2 responsive-column-half">
                                        <img src="assets/images/client-logo-5.png" alt="client-logo" class="filter-gray" />
                                    </div>
                                    {/* <!-- end client-logo-item --> */}
                                    <div class="client-logo-item col-lg-3 mb-2 responsive-column-half">
                                        <img src="assets/images/client-logo-6.png" alt="client-logo" class="filter-gray" />
                                    </div>
                                    {/* <!-- end client-logo-item --> */}
                                </div>
                                {/* <!-- end row --> */}
                            </div>
                            {/* <!-- end client-logo-box --> */}
                            <hr class="border-top-gray" />
                            <div class="value-box pt-60px pb-60px">
                                <h2 class="section-title fs-30 text-center">Our core values</h2>
                                <div class="row pt-50px">
                                    <div class="col-lg-4 responsive-column-half">
                                        <div class="info-box px-3 text-center">
                                            <div class="icon-element icon-element-lg mb-4 shadow-sm mx-auto">
                                            <svg
                                            className="svg-icon-color-1"
                                            width={40}
                                            xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 168.107 168.106"
                                            xmlSpace="preserve"
                                        >
                                            <path d="M143.543 14.515a47.445 47.445 0 0 0-22.952-5.91 47.309 47.309 0 0 0-36.515 17.201C75.336 15.3 62.219 8.605 47.478 8.605c-8.304 0-16.12 2.167-22.925 5.91C9.917 22.592 0 38.157 0 56.08c0 5.122.819 10.047 2.331 14.645 4.465 20.093 28.622 34.735 49.585 51.414 17.196 13.657 32.167 37.362 32.167 37.362s14.512-23.377 31.39-36.77c21.081-16.682 45.768-31.642 50.302-51.997a46.68 46.68 0 0 0 2.333-14.643c-.009-17.934-9.922-33.499-24.565-41.576zm9.422 52.006c-.461 1.445-1.774 2.299-3.185 2.299-.346 0-.724 0-1.05-.131a3.364 3.364 0 0 1-2.135-4.269 26.171 26.171 0 0 0 1.345-8.339 27.327 27.327 0 0 0-14.123-23.901c-4.1-2.298-8.531-3.414-13.227-3.414a3.35 3.35 0 0 1-3.354-3.35c0-1.907 1.485-3.352 3.354-3.352 5.845 0 11.393 1.379 16.477 4.271 10.84 5.909 17.57 17.335 17.57 29.745a32.804 32.804 0 0 1-1.672 10.441z" />
                                        </svg>
                                            </div>
                                            <div class="info-body">
                                                <h3 class="fs-18 pb-3 fw-bold">Adopt a customer-first mindset</h3>
                                                <p>This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>
                                            </div>
                                        </div>
                                    </div>
                                    {/* <!-- end col-lg-4 --> */}
                                    <div class="col-lg-4 responsive-column-half">
                                        <div class="info-box px-3 text-center">
                                            <div class="icon-element icon-element-lg mb-4 shadow-sm mx-auto">
                                                <svg class="svg-icon-color-2" width="40" viewBox="0 -17 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m74.722656 297.265625h-51.734375c-12.675781 0-22.988281 10.3125-22.988281 22.992187v134.375c0 12.675782 10.3125 22.988282 22.988281 22.988282h51.734375c12.679688 0 22.992188-10.3125 22.992188-22.988282v-134.375c0-12.679687-10.3125-22.992187-22.992188-22.992187zm7.992188 157.367187c0 4.402344-3.585938 7.988282-7.992188 7.988282h-51.734375c-4.402343 0-7.988281-3.585938-7.988281-7.988282v-134.375c0-4.40625 3.585938-7.992187 7.988281-7.992187h51.734375c4.40625 0 7.992188 3.585937 7.992188 7.992187zm0 0" /><path d="m212.820312 191.945312h-51.734374c-12.675782 0-22.988282 10.3125-22.988282 22.988282v42.644531c0 4.140625 3.355469 7.5 7.5 7.5 4.140625 0 7.5-3.359375 7.5-7.5v-42.644531c0-4.402344 3.582032-7.988282 7.988282-7.988282h51.734374c4.40625 0 7.988282 3.585938 7.988282 7.988282v239.699218c0 4.402344-3.582032 7.988282-7.988282 7.988282h-51.734374c-4.40625 0-7.988282-3.585938-7.988282-7.988282v-167.054687c0-4.140625-3.359375-7.5-7.5-7.5-4.144531 0-7.5 3.359375-7.5 7.5v167.054687c0 12.675782 10.3125 22.988282 22.988282 22.988282h51.734374c12.675782 0 22.988282-10.3125 22.988282-22.988282v-239.695312c0-12.679688-10.3125-22.992188-22.988282-22.992188zm0 0" /><path d="m350.914062 255.140625h-51.734374c-12.675782 0-22.988282 10.3125-22.988282 22.988281v176.503906c0 12.675782 10.3125 22.988282 22.988282 22.988282h51.734374c12.679688 0 22.992188-10.3125 22.992188-22.988282v-176.503906c0-12.675781-10.3125-22.988281-22.992188-22.988281zm7.992188 199.492187c0 4.402344-3.585938 7.988282-7.992188 7.988282h-51.734374c-4.40625 0-7.988282-3.585938-7.988282-7.988282v-176.503906c0-4.40625 3.582032-7.988281 7.988282-7.988281h51.734374c4.40625 0 7.992188 3.582031 7.992188 7.988281zm0 0" /><path d="m489.011719 139.285156h-51.734375c-12.675782 0-22.992188 10.3125-22.992188 22.988282v214.304687c0 4.140625 3.359375 7.5 7.5 7.5 4.144532 0 7.5-3.359375 7.5-7.5v-214.304687c0-4.40625 3.585938-7.988282 7.992188-7.988282h51.734375c4.402343 0 7.988281 3.582032 7.988281 7.988282v292.359374c0 4.402344-3.585938 7.988282-7.988281 7.988282h-51.734375c-4.40625 0-7.992188-3.585938-7.992188-7.988282v-48.054687c0-4.140625-3.355468-7.5-7.5-7.5-4.140625 0-7.5 3.359375-7.5 7.5v48.054687c0 12.675782 10.316406 22.988282 22.992188 22.988282h51.734375c12.675781 0 22.988281-10.3125 22.988281-22.988282v-292.359374c0-12.675782-10.3125-22.988282-22.988281-22.988282zm0 0" /><path d="m48.855469 157.980469c-25.765625 0-46.730469 20.964843-46.730469 46.730469 0 25.769531 20.964844 46.734374 46.730469 46.734374 25.769531 0 46.734375-20.964843 46.734375-46.734374 0-2.046876-.148438-4.054688-.402344-6.03125l73.683594-56.199219c5.5625 2.34375 11.675781 3.644531 18.082031 3.644531 10.101563 0 19.457031-3.234375 27.109375-8.703125l64.460938 29.496094c2.195312 23.742187 22.21875 42.398437 46.523437 42.398437 25.769531 0 46.734375-20.964844 46.734375-46.730468 0-2.792969-.261719-5.523438-.730469-8.179688l76.027344-63.789062c5.011719 1.84375 10.425781 2.847656 16.066406 2.847656 25.765625 0 46.730469-20.964844 46.730469-46.734375 0-25.765625-20.964844-46.730469-46.730469-46.730469-25.769531 0-46.734375 20.964844-46.734375 46.730469 0 2.792969.261719 5.523437.730469 8.179687l-76.03125 63.789063c-5.007813-1.839844-10.421875-2.847657-16.0625-2.847657-10.101563 0-19.457031 3.234376-27.109375 8.707032l-64.460938-29.5c-2.195312-23.742188-22.214843-42.398438-46.523437-42.398438-25.769531 0-46.730469 20.964844-46.730469 46.730469 0 2.046875.144532 4.058594.402344 6.035156l-73.6875 56.195313c-5.566406-2.34375-11.671875-3.640625-18.082031-3.640625zm414.289062-142.980469c17.496094 0 31.730469 14.234375 31.730469 31.730469 0 17.496093-14.234375 31.734375-31.730469 31.734375-17.234375 0-31.734375-14.5-31.734375-31.734375 0-17.496094 14.234375-31.730469 31.734375-31.730469zm-40.5625 54.925781c.125.21875.261719.429688.390625.644531 2.710938 4.566407 6.199219 8.667969 10.277344 12.070313l-67.640625 56.746094c-.125-.21875-2.175781-3.476563-3.179687-4.816407-2.179688-2.90625-4.691407-5.5625-7.488282-7.898437zm-129.261719 92.191407c.257813-17.007813 14.621094-31.265626 31.726563-31.265626 17.054687 0 31.730469 14.582032 31.730469 31.734376 0 17.496093-14.234375 31.730468-31.730469 31.730468s-31.730469-14.234375-31.730469-31.730468c0-.15625 0-.3125.003906-.46875zm-67.832031-36.328126c3.070313-4.507812 5.386719-9.519531 6.738281-14.804687l54.46875 24.921875c-2.773437 3.996094-4.941406 8.433594-6.359374 13.089844-.203126.660156-.390626 1.324218-.5625 1.992187l-54.46875-24.921875c.0625-.089844.121093-.183594.183593-.277344zm-38.535156-58.128906c17.496094 0 31.730469 14.234375 31.730469 31.730469 0 17.09375-14.664063 31.734375-31.730469 31.734375-17.074219 0-31.734375-14.5625-31.734375-31.734375 0-17.496094 14.234375-31.730469 31.734375-31.730469zm-41.597656 53.019532c.089843.171874.191406.34375.28125.515624 2.507812 4.742188 5.832031 9.054688 9.789062 12.675782l-64.972656 49.554687c-1.203125-2.34375-2.617187-4.59375-4.195313-6.699219-1.75-2.335937-3.710937-4.515624-5.875-6.496093zm-77.589844 58.824218c7.699219 5.859375 12.824219 15.085938 12.824219 25.207032 0 17.5-14.234375 31.734374-31.734375 31.734374-17.496094 0-31.730469-14.234374-31.730469-31.734374 0-17.496094 14.234375-31.730469 31.730469-31.730469 6.976562 0 13.566406 2.449219 18.910156 6.523437zm0 0" /></svg>
                                            </div>
                                            <div class="info-body">
                                                <h3 class="fs-18 pb-3 fw-bold">Learn, share, grow</h3>
                                                <p>This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>
                                            </div>
                                        </div>
                                    </div>
                                    {/* <!-- end col-lg-4 --> */}
                                    <div class="col-lg-4 responsive-column-half">
                                        <div class="info-box px-3 text-center">
                                            <div class="icon-element icon-element-lg mb-4 shadow-sm mx-auto">
                                                <svg class="svg-icon-color-3" width="40" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><g id="XMLID_420_"><g id="XMLID_350_"><path id="XMLID_353_" d="m256.004 102c-99.804 0-181 81.196-181 181s81.196 181 181 181 181-81.196 181-181-81.196-181-181-181zm0 342c-88.776 0-161-72.224-161-161s72.224-161 161-161 161 72.224 161 161-72.224 161-161 161z" /><path id="XMLID_356_" d="m337.003 188.475-111.86 59.534c-1.757.935-3.194 2.373-4.129 4.129l-59.534 111.86c-2.066 3.882-1.354 8.659 1.756 11.77 1.923 1.922 4.482 2.929 7.074 2.929 1.601 0 3.213-.384 4.695-1.173l111.86-59.534c1.757-.935 3.194-2.373 4.129-4.129l59.534-111.86c2.066-3.882 1.354-8.659-1.756-11.77-3.11-3.109-7.886-3.821-11.769-1.756zm-80.999 116.525c-12.131 0-22-9.869-22-22s9.869-22 22-22 22 9.869 22 22-9.869 22-22 22zm-38.123-4.406c4.188 9.038 11.492 16.342 20.53 20.53l-43.887 23.357zm76.246-35.188c-4.188-9.038-11.492-16.342-20.53-20.53l43.887-23.357z" /><path id="XMLID_372_" d="m447.269 156.989c-3.043-4.609-9.248-5.879-13.855-2.835-4.609 3.043-5.878 9.246-2.835 13.855 54.379 82.361 43.113 192.875-26.789 262.777-39.475 39.474-91.96 61.214-147.786 61.214s-108.311-21.74-147.785-61.214c-39.475-39.475-61.215-91.96-61.215-147.786s21.74-108.311 61.215-147.785c69.659-69.659 179.923-81.088 262.182-27.18 4.62 3.029 10.818 1.737 13.845-2.882s1.737-10.818-2.882-13.845c-28.275-18.531-60.421-30.442-93.754-35.088 3.421-5.605 5.395-12.186 5.395-19.219 0-20.402-16.598-37-37-37s-37 16.598-37 37c0 7.021 1.966 13.591 5.376 19.19-49.117 6.844-95.087 29.666-130.304 64.882-43.253 43.252-67.073 100.759-67.073 161.927s23.82 118.675 67.072 161.927c43.253 43.253 100.76 67.073 161.928 67.073s118.675-23.82 161.928-67.073c37.528-37.528 60.976-87.242 66.024-139.983 4.982-52.034-8.048-104.579-36.687-147.955zm-191.265-136.989c9.374 0 17 7.626 17 17s-7.626 17-17 17-17-7.626-17-17 7.626-17 17-17z" /><path id="XMLID_373_" d="m256.004 178.543c5.523 0 10-4.477 10-10v-12.543c0-5.523-4.477-10-10-10s-10 4.477-10 10v12.543c0 5.523 4.477 10 10 10z" /><path id="XMLID_374_" d="m256.004 387.457c-5.523 0-10 4.477-10 10v12.543c0 5.523 4.477 10 10 10s10-4.477 10-10v-12.543c0-5.523-4.477-10-10-10z" /><path id="XMLID_376_" d="m360.461 283c0 5.523 4.477 10 10 10h12.543c5.523 0 10-4.477 10-10s-4.477-10-10-10h-12.543c-5.523 0-10 4.477-10 10z" /><path id="XMLID_377_" d="m141.547 273h-12.543c-5.523 0-10 4.477-10 10s4.477 10 10 10h12.543c5.523 0 10-4.477 10-10s-4.477-10-10-10z" /><path id="XMLID_378_" d="m410.784 138.07c2.64 0 5.21-1.07 7.08-2.93 1.86-1.86 2.93-4.44 2.93-7.07s-1.07-5.21-2.93-7.07c-1.87-1.86-4.44-2.93-7.08-2.93-2.63 0-5.21 1.07-7.07 2.93s-2.93 4.44-2.93 7.07 1.07 5.21 2.93 7.07 4.44 2.93 7.07 2.93z" /></g></g></svg>
                                            </div>
                                            <div class="info-body">
                                                <h3 class="fs-18 pb-3 fw-bold">Be transparent</h3>
                                                <p>This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>
                                            </div>
                                        </div>
                                    </div>
                                    {/* <!-- end col-lg-4 --> */}
                                    <div class="col-lg-4 responsive-column-half">
                                        <div class="info-box px-3 text-center">
                                            <div class="icon-element icon-element-lg mb-4 shadow-sm mx-auto">
                                                <svg class="svg-icon-color-4" width="40" viewBox="0 0 511.765 511.765" xmlns="http://www.w3.org/2000/svg"><path d="m491.945 353.116c.15-.305.283-.617.389-.937 30.321-69.981 24.903-155.775-14.478-221.349 21.024-25.133-6.285-61.125-36.149-47.199-5.248-5.363-10.758-10.503-16.468-15.363 11.208-30.468-26.198-55.462-49.494-32.698-67.711-34.191-151.928-33.758-219.265 1.157-23.954-25.385-64.183 1.833-49.68 33.683-8.306 7.263-16.168 15.109-23.417 23.406-6.259 7.164 5.08 16.983 11.296 9.869 6.744-7.718 14.051-15.019 21.769-21.785 14.925 10.765 36.566 5.281 44.754-11.151 27.021 7.778 52.825 18.941 76.87 33.268-2.013 7.259-1.191 15.366 2.263 22.067-19.79 17.095-37.612 36.433-53.088 57.626-14.287-4.317-30.035 2.909-36.248 16.439-31.507.169-63.112 3.558-94.142 10.117-2.74-5.106-6.894-9.337-11.93-12.171 5.83-22.735 15.064-44.362 27.502-64.359 4.956-7.97-7.554-16.255-12.737-7.923-13.224 21.26-23.046 44.251-29.258 68.419-35.93.61-41.962 50.316-7.757 59.878-.504 70.742 30.082 139.52 83.12 186.372-11.064 29.887 24.855 54.539 48.559 33.459 69.178 36.785 156.607 36.79 225.789.012 24.109 23.844 62.723-2.794 49.143-33.965 27.335-24.293 48.57-54.006 62.657-86.872zm-9.45-16.774c-20.761-20.626-43.481-39.337-67.738-55.762 3.445-10.791.451-22.932-7.594-30.898 20.398-33.915 36.411-70.281 47.683-108.27 3.756.013 7.367-.68 10.694-1.934 34.539 58.333 40.856 133.543 16.955 196.864zm-345.126 84.784c-6.312-61.825 4.764-123.504 32.177-179.243 17.03 5.314 35.437-5.885 38.744-23.366 51.715 6.375 101.522 21.558 148.264 45.203-2.248 8.738-.387 18.373 4.968 25.642-52.687 66.749-122.234 116.976-201.83 145.763-4.823-7.637-12.934-12.98-22.323-13.999zm233.227-149.806c0-8.499 6.864-15.413 15.301-15.413 20.298.845 20.292 29.984 0 30.826-8.437 0-15.301-6.914-15.301-15.413zm99.617-160.32c0 8.499-6.864 15.413-15.301 15.413-20.299-.845-20.294-29.984 0-30.826 8.437 0 15.301 6.914 15.301 15.413zm-39.967-17.632c-10.366 14.565-5.555 35.62 10.073 44.277-10.919 36.644-26.399 71.722-46.092 104.441-4.117-1.184-8.687-1.478-12.92-.83-20.584-43.955-50.535-83.546-87.123-115.17 3.457-6.705 4.281-14.798 2.265-22.067 23.159-13.798 47.966-24.672 73.907-32.402 8.622 16.646 31.751 21.346 46.11 8.885 4.759 4.094 9.366 8.395 13.78 12.866zm-178.287 18.793c0-8.499 6.864-15.413 15.301-15.413 20.299.845 20.293 29.983 0 30.825-8.437 0-15.301-6.914-15.301-15.412zm145.068-70.379c8.438 0 15.302 6.914 15.302 15.413-.842 20.447-29.764 20.442-30.603 0 0-8.499 6.864-15.413 15.301-15.413zm-262.94 30.825c-8.437 0-15.301-6.914-15.301-15.413.842-20.448 29.764-20.441 30.603 0 0 8.499-6.865 15.413-15.302 15.413zm30.273-16.563c-.078-2.082-.364-4.109-.84-6.065 62.783-32.467 141.216-33.033 204.483-1.51-.813 2.719-1.258 5.594-1.273 8.571-27.286 8.072-53.377 19.474-77.735 33.961-11.708-12.111-31.757-12.115-43.469-.001-25.383-15.098-52.63-26.834-81.166-34.956zm85.905 81.282c10.142 6.922 23.845 6.923 33.988.001 34.932 30.161 63.548 67.916 83.259 109.841-1.336 1.027-2.584 2.164-3.732 3.395-49.279-24.995-101.867-40.887-156.491-47.281-1.475-4.464-3.951-8.472-7.165-11.753 14.66-19.915 31.484-38.104 50.141-54.203zm-71.738 60.095c8.437 0 15.301 6.914 15.301 15.413-.842 20.448-29.764 20.442-30.603 0 .001-8.499 6.865-15.413 15.302-15.413zm-163.668 27.218c0-8.499 6.864-15.413 15.301-15.413 20.298.845 20.292 29.983 0 30.825-8.436.001-15.301-6.913-15.301-15.412zm22.817 29.465s22.781-4.968 22.781-29.267c0 0 58.456-9.344 87.867-9.672.552 7.266 3.655 13.823 8.414 18.771-29.228 58.801-41.017 123.993-34.231 189.321-2.922 1.218-5.612 2.883-7.99 4.907-48.931-43.834-76.841-117.278-76.841-174.06zm96.411 212.665c-8.437 0-15.301-6.914-15.301-15.413s6.864-15.413 15.301-15.413c8.324 0 15.302 7.21 15.302 15.413 0 8.499-6.865 15.413-15.302 15.413zm133.173 19.877c-36.401 0-72.397-8.718-104.569-25.269 1.306-3.743 1.882-8.007 1.628-11.979 82.338-29.679 154.289-81.546 208.795-150.513 4.478 2.1 9.447 3.064 14.384 2.805 3.332 12.367 5.965 24.984 7.831 37.6 1.429 9.658 16.272 7.5 14.839-2.194-1.981-13.396-4.786-26.794-8.347-39.916 1.793-1.116 3.459-2.417 4.977-3.872 25.028 16.991 48.362 36.496 69.501 58.083-13.11 28.981-32.191 55.222-56.434 76.905-2.397-1.8-5.07-3.248-7.939-4.278 1.474-13.943 2.116-28.021 1.9-41.945-.152-9.798-15.151-9.568-14.998.232.208 13.384-.412 26.917-1.836 40.32-17.689 2.813-29.258 21.155-24.384 38.345-32.374 16.814-68.649 25.676-105.348 25.676zm134.49-18.573c-8.437 0-15.301-6.914-15.301-15.413.842-20.448 29.764-20.441 30.603 0 0 8.499-6.864 15.413-15.302 15.413z" /></svg>
                                            </div>
                                            <div class="info-body">
                                                <h3 class="fs-18 pb-3 fw-bold">Empower people to deliver outstanding results</h3>
                                                <p>This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>
                                            </div>
                                        </div>
                                    </div>
                                    {/* <!-- end col-lg-4 --> */}
                                    <div class="col-lg-4 responsive-column-half">
                                        <div class="info-box px-3 text-center">
                                            <div class="icon-element icon-element-lg mb-4 shadow-sm mx-auto">
                                                <svg class="svg-icon-color-5" width="40" viewBox="0 0 496 496" xmlns="http://www.w3.org/2000/svg"><path d="m496 208h-17.976562l-26.792969 14.097656c-3.015625-1.664062-6.152344-3.074218-9.535157-3.921875l-33.703124-8.433593v-3.832032c14.910156-10.40625 24.007812-27.597656 24.007812-45.910156v-13.984375c0-31.078125-24.351562-57.085937-54.28125-57.992187-1.519531-.039063-3.015625.015624-4.519531.089843l8.738281-20.457031-42.363281-42.351562-24.972657 16.253906c-9.488281-5.34375-19.523437-9.503906-29.945312-12.421875l-6.167969-29.136719h-60.96875l-6.167969 29.144531c-10.421874 2.917969-20.464843 7.078125-29.945312 12.421875l-24.972656-16.253906-42.328125 42.328125 8.589843 20.472656c-.328124-.015625-.640624-.074219-.96875-.082031-15.320312-.445312-29.75 5.144531-40.710937 15.785156-10.976563 10.632813-17.015625 24.902344-17.015625 40.183594v16c0 18.3125 9.089844 35.496094 24 45.902344v3.839844l-21.984375 5.496093-39.847656-23.238281h-26.167969v165.769531l78.703125 26.230469h15.328125l1.914062-6.703125 24.125 14.480469c.394532 13.117187 10.914063 23.671875 24.019532 24.132812.460937 13.050782 10.949218 23.539063 24 24 .460937 13.050782 10.949218 23.539063 24 24 .476562 13.355469 11.414062 24.089844 24.878906 24.089844 5 0 9.902344-1.558594 14.023438-4.335938l7.085937 6.257813c4.441406 3.910156 10.160156 6.078125 16.097656 6.078125h1.480469c13.3125 0 24.144531-10.761719 24.3125-24.03125 13.167969-.167969 23.832031-10.832031 24-24 13.175781-.167969 23.847656-10.847656 24-24.023438 13.296875-.3125 24.03125-11.175781 24.03125-24.546874 0-.804688-.039062-1.597657-.121094-2.398438l144.121094-72.054688zm-80-61.984375v13.984375c0 14.199219-7.671875 27.464844-20.023438 34.617188l-4 2.3125.03125 25.316406 39.96875 9.984375-87.976562 46.304687v-21.550781c0-10.753906-4.34375-20.601563-11.457031-27.878906l27.457031-6.859375v-25.335938l-3.992188-2.308594c-12.34375-7.136718-20.007812-20.402343-20.007812-34.601562v-16c0-10.910156 4.3125-21.113281 12.152344-28.710938 7.832031-7.59375 18.222656-11.6875 29.085937-11.265624 21.371094.640624 38.761719 19.480468 38.761719 41.992187zm-155.9375 126.230469-36.214844-11.574219c-16.503906-5.289063-34.367187-6.078125-51.261718-2.335937l-4.585938 1.015624v-2.367187c0-11.03125 7.480469-20.609375 18.175781-23.289063l45.824219-11.449218v-25.335938l-3.992188-2.308594c-12.34375-7.136718-20.007812-20.402343-20.007812-34.601562v-16c0-10.910156 4.3125-21.113281 12.152344-28.710938 7.832031-7.59375 18.289062-11.6875 29.085937-11.265624 21.371094.640624 38.761719 19.480468 38.761719 41.992187v13.984375c0 14.199219-7.671875 27.464844-20.023438 34.617188l-4 2.3125.03125 25.316406 45.816407 11.449218c10.695312 2.679688 18.175781 12.257813 18.175781 23.289063v29.976563l-1.976562 1.039062h-4.902344l-49.183594-14.054688c-3.875-1.097656-7.867188-1.546874-11.875-1.699218zm-90.269531-152.511719c15.023437-29.109375 45.261719-47.734375 78.207031-47.734375 32.632812 0 62.742188 18.375 77.878906 47.089844-3.808594 7.636718-5.878906 16.09375-5.878906 24.910156v16c0 18.3125 9.089844 35.496094 24 45.902344v3.839844l-32 8.011718-32.007812-8.011718v-3.832032c14.910156-10.40625 24.007812-27.597656 24.007812-45.910156v-13.984375c0-31.078125-24.351562-57.085937-54.28125-57.992187-15.3125-.449219-29.75 5.144531-40.710938 15.785156-10.96875 10.640625-17.007812 24.910156-17.007812 40.191406v16c0 18.3125 9.089844 35.496094 24 45.902344v3.839844l-32 8.011718-32.007812-8.011718v-3.832032c14.910156-10.40625 24.007812-27.597656 24.007812-45.910156v-13.984375c0-9.414063-2.265625-18.351563-6.207031-26.28125zm-36.792969-48.382813 25.558594-25.558593 22.496094 14.640625 4.304687-2.640625c10.785156-6.609375 22.441406-11.449219 34.640625-14.367188l4.921875-1.179687 5.566406-26.246094h35.03125l5.550781 26.238281 4.921876 1.175781c12.199218 2.921876 23.855468 7.761719 34.640624 14.371094l4.304688 2.640625 22.496094-14.640625 25.527344 25.535156-9.074219 21.246094c-6.175781 2.65625-11.902344 6.394532-16.886719 11.226563-.328125.320312-.609375.664062-.929688.992187-18.847656-29.960937-52.167968-48.785156-88.070312-48.785156-36.160156 0-69.574219 19-88.375 49.265625-5.03125-5.265625-11.015625-9.53125-17.648438-12.511719zm-29 150.894532v-25.335938l-3.992188-2.308594c-12.34375-7.136718-20.007812-20.402343-20.007812-34.601562v-16c0-10.910156 4.3125-21.113281 12.152344-28.710938 7.832031-7.59375 18.246094-11.6875 29.085937-11.265624 21.371094.640624 38.761719 19.480468 38.761719 41.992187v13.984375c0 14.199219-7.671875 27.464844-20.023438 34.617188l-4 2.3125.03125 25.316406 27.449219 6.859375c-7.113281 7.277343-11.457031 17.125-11.457031 27.878906v5.925781l-24.953125 5.546875.648437-2.265625.304688-14.785156-42.191406-24.613281zm-88-14.246094h5.832031l57.535157 33.558594-29.03125 116.121094-34.335938-11.449219zm65.296875 160-15.746094-5.246094 28.203125-112.800781 18.246094 10.640625v2.285156l-30.03125 105.121094zm54.703125 23.03125c0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625-4.945312 0-8.96875-4.023438-8.96875-8.96875zm24 24c0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625-4.945312 0-8.96875-4.023438-8.96875-8.96875zm24 24c0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625-4.945312 0-8.96875-4.023438-8.96875-8.96875zm32.96875 32.96875c-4.945312 0-8.96875-4.023438-8.96875-8.96875 0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625zm38.6875 8h-1.480469c-2.039062 0-3.992187-.742188-5.511719-2.089844l-5.984374-5.277344 6.007812-6.007812c2.144531-2.144531 3.808594-4.695312 5.039062-7.433594l7.273438 6.054688c1.902344 1.585937 3 3.921875 3 6.410156 0 4.601562-3.742188 8.34375-8.34375 8.34375zm71.742188-72h-1.382813c-2.007813 0-3.96875-.710938-5.503906-2l-43.382813-36.152344-10.25 12.296875 42.128906 35.101563c1.894532 1.585937 2.992188 3.921875 2.992188 6.410156 0 4.601562-3.742188 8.34375-8.34375 8.34375h-1.746094c-1.941406 0-3.839844-.6875-5.335937-1.9375l-43.453125-36.207031-10.25 12.296875 42.128906 35.105468c1.902344 1.574219 3 3.910157 3 6.398438 0 4.601562-3.742188 8.34375-8.34375 8.34375-3.070312 0-6.0625-1.089844-8.417969-3.046875l-17.855469-14.882813c-3.933593-8.039062-12.015624-13.644531-21.480468-13.980468-.460938-13.050782-10.949219-23.539063-24-24-.460938-13.050782-10.949219-23.539063-24-24-.46875-13.355469-11.40625-24.089844-24.871094-24.089844-6.574219 0-13.007812 2.664062-17.65625 7.3125l-14.0625 14.0625c-.839844.839844-1.535156 1.792969-2.242188 2.738281l-24.574218-14.746093 21.542968-75.398438 54.007813-12.007812c14.167969-3.167969 29.105469-2.480469 42.921875 1.941406l11.785156 3.769531-39.953125 19.976563c-9.128906 4.574218-14.800781 13.757812-14.800781 23.96875v1.582031c0 14.777343 12.023438 26.800781 26.800781 26.800781 4.855469 0 9.632813-1.320312 13.785157-3.824219l29.789062-17.863281c5.984375-3.585938 14.015625-2.800781 19.183594 1.847656l67.59375 60.832032c1.808594 1.640624 2.847656 3.96875 2.847656 6.40625 0 4.746093-3.855469 8.601562-8.601562 8.601562zm152.601562-92.945312-134.777344 67.394531c-.4375-.464844-.894531-.90625-1.375-1.335938l-67.582031-60.832031c-5.921875-5.34375-13.578125-8.28125-21.554687-8.28125-5.832032 0-11.558594 1.585938-16.574219 4.59375l-29.800781 17.871094c-1.671876 1.007812-3.582032 1.535156-5.535157 1.535156-5.960937 0-10.800781-4.839844-10.800781-10.800781v-1.582031c0-4.121094 2.289062-7.816407 5.96875-9.65625l44.625-22.3125c7.636719-3.824219 16.703125-4.679688 24.933594-2.3125l50.273437 14.359374 12.175781.304688 150.023438-78.960938zm0 0" /></svg>
                                            </div>
                                            <div class="info-body">
                                                <h3 class="fs-18 pb-3 fw-bold">Keep community at our center</h3>
                                                <p>This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>
                                            </div>
                                        </div>
                                    </div>
                                    {/* <!-- end col-lg-4 --> */}
                                    <div class="col-lg-4 responsive-column-half">
                                        <div class="info-box px-3 text-center">
                                            <div class="icon-element icon-element-lg mb-4 shadow-sm mx-auto">
                                            <svg
                                                className="svg-icon-color-6"
                                                width={40}
                                                xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 512 512"
                                                xmlSpace="preserve"
                                            >
                                                <path d="M402.034 77.436a241.872 241.872 0 0 0-56.135-75.499 7.504 7.504 0 0 0-10.595.531 7.503 7.503 0 0 0 .531 10.595 226.83 226.83 0 0 1 52.647 70.806c1.79 3.767 6.277 5.324 9.993 3.559a7.5 7.5 0 0 0 3.559-9.992zM359.714 88.616a199.058 199.058 0 0 0-36.707-49.369 7.502 7.502 0 1 0-10.51 10.705 183.994 183.994 0 0 1 33.938 45.645 7.5 7.5 0 0 0 6.647 4.012c5.564 0 9.279-5.958 6.632-10.993zM72.529 318.782a226.994 226.994 0 0 1-29.8-32.818 226.837 226.837 0 0 1-22.845-37.987 7.501 7.501 0 0 0-13.552 6.434 241.86 241.86 0 0 0 56.134 75.497 7.468 7.468 0 0 0 5.029 1.938c6.807-.001 10.144-8.443 5.034-13.064zM95.87 281.895a184.026 184.026 0 0 1-33.937-45.646 7.501 7.501 0 0 0-13.278 6.982 199.087 199.087 0 0 0 36.707 49.37 7.5 7.5 0 0 0 10.508-10.706z" />
                                                <path d="m499.781 371.92-13.367-17.98c1.904-7.314 8.028-25.754 4.968-51.339-3.324-27.768-11.94-32.114-14.678-48.103-6.713-39.192 11.373-92.947 15.066-103.325.155-.437.247-.707.277-.803 7.899-24.969 13.117-41.466-12.029-53.564-22.497-10.822-46.173-1.213-60.314 24.482-.866 1.574-19.008 34.933-26.809 80.441l-92.57-124.504a7.498 7.498 0 0 0-10.495-1.544 7.5 7.5 0 0 0-1.544 10.494l103.578 139.309c4.091 5.502 12.838 3.055 13.486-3.765 4.912-51.683 26.576-91.522 27.497-93.194 3.853-7.006 18.18-29.018 40.668-18.198 14.204 6.834 11.611 10.803 4.122 35.816-5.497 15.446-22.878 69.093-15.72 110.888 3.435 20.051 11.442 21.223 14.568 47.355 1.252 10.46.869 21.125-1.127 31.831-.482 2.363-3.211 13.553-13.584 26.015a7.502 7.502 0 0 0 .967 10.564c3.189 2.655 7.919 2.21 10.564-.967a90.499 90.499 0 0 0 3.935-5.084l10.503 14.126c6.101 8.205 4.388 19.843-3.815 25.942l-15.78 11.733c-5.794 4.308-2.714 13.52 4.48 13.52 4.236 0 4.927-2.383 20.251-13.215 14.841-11.036 17.938-32.09 6.902-46.931zM450.504 441.011a7.497 7.497 0 0 0-10.494-1.544l-72.417 53.844c-8.293 6.165-20.053 4.433-26.217-3.855-2.995-3.154-13.062-22.889-37.872-30.259-44.591-13.246-59.616-46.662-68.075-55.748a7.502 7.502 0 0 0-12.038 8.952c7.916 8.427 25.751 46.297 75.842 61.176 19.255 5.72 26.879 21.366 30.107 24.829 11.08 14.901 32.252 18.061 47.205 6.943l72.418-53.844a7.501 7.501 0 0 0 1.541-10.494zM102.826 250.247l-.048-.065.048.065zM102.755 250.152c-.002-.003-.002-.003 0 0z" />
                                                <path d="M317.375 203.536 218.852 71.023v-.001c-7.985-10.737-5.469-26.482 6.624-33.883 10.155-6.217 23.866-3.343 31.216 6.541l10.675 14.358a7.5 7.5 0 0 0 10.494 1.544 7.5 7.5 0 0 0 1.544-10.494L268.73 34.73c-19.083-25.665-59.451-18.936-68.091 12.115-21.714-23.89-61.463-12.786-66.591 19.229-1.479 9.233.392 18.453 5.278 26.291-31.77-.685-50.429 35.775-31.132 61.726l11.734 15.782c-32.355-.346-49.896 36.267-30.972 61.72l13.804 18.566-.005-.006.023.03-.016-.021.232.312-.168-.226.208.28a.022.022 0 0 0 .004.005l99.43 133.73a7.484 7.484 0 0 0 6.025 3.025c6.073 0 9.723-6.987 6.014-11.976C77.984 191.69 115.436 242.065 100.994 222.641c-12.373-16.643.388-39.717 20.698-37.654 11.121 1.128 16.275 9.205 16.393 9.308l68.253 91.798a7.487 7.487 0 0 0 6.025 3.025c6.089 0 9.715-6.998 6.014-11.976L120.23 145.138c-7.48-10.06-6.135-24.148 3.063-32.073 10.362-8.927 25.938-7.154 34.018 3.712l92.621 124.573a7.487 7.487 0 0 0 6.025 3.025 7.5 7.5 0 0 0 6.013-11.975C253.8 221.407 158.606 93.373 153.174 86.066c-7.825-10.523-5.477-25.424 5.307-33.021 10.247-7.214 24.856-4.645 32.568 5.727l114.287 153.713a7.5 7.5 0 0 0 10.494 1.544 7.497 7.497 0 0 0 1.545-10.493zm-214.326 47.012c.073.097.054.072 0 0z" />
                                            </svg>
                                            </div>
                                            <div class="info-body">
                                                <h3 class="fs-18 pb-3 fw-bold">Be flexible and inclusive</h3>
                                                <p>This is just a simple text made for this unique and awesome template, you can replace it with any text.</p>
                                            </div>
                                        </div>
                                    </div>
                                    {/* <!-- end col-lg-4 --> */}
                                </div>
                                {/* <!-- end row --> */}
                            </div>
                            {/* <!-- end value-box --> */}
                            <hr class="border-top-gray" />
                            <div class="client-logo-box pt-60px">
                                <h2 class="section-title fs-30 text-center">Investors</h2>
                                <div class="row justify-content-center text-center pt-50px">
                                    <div class="col-lg-3 responsive-column-half">
                                        <a href="#" class="client-logo-item mb-30px bg-white shadow-sm p-3 rounded-rounded d-block hover-y">
                                            <img src="assets/images/client-logo.png" alt="client-logo" />
                                        </a>
                                    </div>
                                    {/* <!-- end col-lg-3 --> */}
                                    <div class="col-lg-3 responsive-column-half">
                                        <a href="#" class="client-logo-item mb-30px bg-white shadow-sm p-3 rounded-rounded d-block hover-y">
                                            <img src="assets/images/client-logo-2.png" alt="client-logo" />
                                        </a>
                                    </div>
                                    {/* <!-- end col-lg-3 --> */}
                                    <div class="col-lg-3 responsive-column-half">
                                        <a href="#" class="client-logo-item mb-30px bg-white shadow-sm p-3 rounded-rounded d-block hover-y">
                                            <img src="assets/images/client-logo-3.png" alt="client-logo" />
                                        </a>
                                    </div>
                                    {/* <!-- end col-lg-3 --> */}
                                    <div class="col-lg-3 responsive-column-half">
                                        <a href="#" class="client-logo-item mb-30px bg-white shadow-sm p-3 rounded-rounded d-block hover-y">
                                            <img src="assets/images/client-logo-4.png" alt="client-logo" />
                                        </a>
                                    </div>
                                    {/* <!-- end col-lg-3 --> */}
                                    <div class="col-lg-3 responsive-column-half">
                                        <a href="#" class="client-logo-item mb-30px bg-white shadow-sm p-3 rounded-rounded d-block hover-y">
                                            <img src="assets/images/client-logo-5.png" alt="client-logo" />
                                        </a>
                                    </div>
                                    {/* <!-- end col-lg-3 --> */}
                                    <div class="col-lg-3 responsive-column-half">
                                        <a href="#" class="client-logo-item mb-30px bg-white shadow-sm p-3 rounded-rounded d-block hover-y">
                                            <img src="assets/images/client-logo-6.png" alt="client-logo" />
                                        </a>
                                    </div>
                                    {/* <!-- end col-lg-3 --> */}
                                </div>
                                {/* <!-- end row --> */}
                            </div>
                            {/* <!-- end client-logo-box --> */}
                        </div>
                        {/* <!-- end about-box --> */}
                    </div>
                    {/* <!-- end container --> */}
                </section>
                {/* <!-- end about-area --> */}



                {/* <!-- ================================
         START CAREER AREA
================================= --> */}
                <section class="career-area pb-70px">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-5">
                                <div class="career-content mb-50px">
                                    <h2 class="section-title fs-30 pb-3">Where we work</h2>
                                    <p class="section-desc pb-4">We’re proud to be a remote first company working across many timezones, with offices in New York, London, Austin and Munich *</p>
                                    <div class="btn-box pb-3">
                                        <a href="careers.html" class="btn theme-btn mr-2 mb-2">Current job openings <i class="la la-arrow-right icon ml-1"></i></a>
                                        <a href="contact.html" class="btn theme-btn theme-btn-outline theme-btn-outline-gray mb-2">Contact us <i class="la la-arrow-right icon ml-1"></i></a>
                                    </div>
                                    <hr class="border-top-gray" />
                                    <div class="address-box d-flex pt-4">
                                        <img src="assets/images/img-loading.png" data-src="assets/images/illo-new-york.png" alt="Illustration of New York skyline" class="mr-4 img-fluid rounded-rounded lazy" />
                                        <div>
                                            <h5 class="mb-2 text-gray fs-18">HQ</h5>
                                            <h3 class="mb-4 fs-20">New York</h3>
                                            <p>
                                                Disilab Inc.<br />
                                                110 Colin Street<br />
                                                12th Floor<br />
                                                New York, NY 10038
                                            </p>
                                        </div>
                                    </div>
                                    <p class="fs-14 pt-4">* All employees are currently <a href="blog-grid-no-sidebar.html" class="btn-text">working fully remote <i class="la la-arrow-right icon ml-1"></i></a></p>
                                </div>
                                {/* <!-- end career-content --> */}
                            </div>
                            {/* <!-- end col-lg-5 --> */}
                            <div class="col-lg-7">
                                <div class="work-content d-flex flex-wrap align-items-center justify-content-end">
                                    <div class="work-content-card d-flex align-items-center justify-content-center text-center bg-diagonal-gradient-primary js-tilt" data-toggle="tooltip" data-placement="top" title="37%">
                                        <div class="work-content-card-inner">
                                        <svg
                                        aria-hidden="true"
                                        className="mb-2"
                                        width={22}
                                        height={22}
                                        viewBox="0 0 18 18"
                                    
                                    >
                                        <rect x={1} y={3} width={16} height={12} rx={2} fill="#FEFEFE" />
                                        <path d="M1 5c0-1.1.9-2 2-2h6v5H1V5z" fill="#0B327C" />
                                        <path
                                        d="M17 5H9v1h8V5zm-2-2H9v1h7.73A2 2 0 0 0 15 3zm2 4H9v1h8V7zm0 2v1H1V9h16zm0 2v1H1v-1h16zm-.27 3a2 2 0 0 0 .27-.99V13H1c0 .36.1.7.27 1h15.46z"
                                        fill="red"
                                        />
                                    </svg>
                                            <h3 class="fs-17 text-white fw-semi-bold">New York</h3>
                                        </div>
                                    </div>
                                    {/* <!-- end work-content-card --> */}
                                    <div class="work-content-card remote d-flex align-items-center justify-content-center text-center bg-diagonal-gradient-primary js-tilt" data-toggle="tooltip" data-placement="top" title="44%">
                                        <div class="work-content-card-inner">
                                            <svg aria-hidden="true" class="mb-2" width="22" height="22" viewBox="0 0 18 18"><path d="M9 1a8 8 0 100 16A8 8 0 009 1zM8 15.32a6.4 6.4 0 01-5.23-7.75L7 11.68v.8c0 .88.12 1.32 1 1.32v1.52zm5.72-2c-.2-.66-1-1.32-1.72-1.32h-1v-2c0-.44-.56-1-1-1H6V7h1c.44 0 1-.56 1-1V5h2c.88 0 1.4-.72 1.4-1.6v-.33a6.4 6.4 0 012.32 10.24v.01z" fill="white"></path>
                                            </svg>
                                            <h3 class="fs-19 text-white fw-semi-bold">Remote</h3>
                                        </div>
                                    </div>
                                    {/* <!-- end work-content-card --> */}
                                </div>
                                {/* <!-- end work-content --> */}
                                <div class="work-content d-flex flex-wrap align-items-center justify-content-center">
                                    <div class="work-content-card austin d-flex align-items-center justify-content-center text-center bg-diagonal-gradient-primary js-tilt" data-toggle="tooltip" data-placement="top" title="3%">
                                        <div class="work-content-card-inner">
                                            <svg aria-hidden="true" class="mb-2" width="22" height="22" viewBox="0 0 18 18"><rect x="1" y="3" width="16" height="12" rx="2" fill="#FEFEFE"></rect><path d="M1 5c0-1.1.9-2 2-2h6v5H1V5z" fill="#0B327C"></path><path d="M17 5H9v1h8V5zm-2-2H9v1h7.73A2 2 0 0015 3zm2 4H9v1h8V7zm0 2v1H1V9h16zm0 2v1H1v-1h16zm-.27 3a2 2 0 00.27-.99V13H1c0 .36.1.7.27 1h15.46z" fill="red"></path></svg>
                                            <h3 class="fs-15 text-white">Austin</h3>
                                        </div>
                                    </div>
                                    {/* <!-- end work-content-card --> */}
                                    <div class="work-content-card london d-flex align-items-center justify-content-center text-center bg-diagonal-gradient-primary js-tilt" data-toggle="tooltip" data-placement="top" title="14%">
                                        <div class="work-content-card-inner">
                                            <svg aria-hidden="true" class="mb-2" width="22" height="22" viewBox="0 0 18 18"><rect x="1" y="3" width="16" height="12" rx="2" fill="#3F51B5"></rect><path d="M7 5.28L4.08 3H3a2 2 0 00-.89.2L7 7H5.39L1.34 3.88l.02-.02a2 2 0 00-.3.65L4.31 7H1v1h7V3H7v2.28zM1 11v-1h7v5H7v-3.14L2.9 15a2 2 0 01-.8-.21L7 11H5.38l-4.03 3.11A2 2 0 011 13v-.33L3.14 11H1zm9 4h1v-2.3l2.95 2.3H15a2 2 0 00.9-.22L11 11h1.64l4.02 3.1c.13-.18.23-.4.28-.62L13.71 11H17v-1h-7v5zm1-12v3.15L15.11 3c.29.02.55.1.8.22L11 7h1.64l4.02-3.1A2 2 0 0117 5v.33L14.86 7H17v1h-7V3h1z" fill="#FEFEFE"></path><path d="M10 3H8v5H1v2h7v5h2v-5h7V8h-7V3zm-7.89.2a2 2 0 00-.75.66l-.02.02L5.4 7H7v-.02L2.11 3.21zm-.77 10.91a2 2 0 00.76.68L7 11H5.38l-4.03 3.11zm15.32-.01a2 2 0 01-.76.68L11 11h1.64l4.02 3.1zM12.64 7l4.02-3.1a2 2 0 00-.76-.68L11 7h1.64z" fill="#E53935"></path></svg>
                                            <h3 class="fs-17 text-white">London</h3>
                                        </div>
                                    </div>
                                    {/* <!-- end work-content-card --> */}
                                    <div class="work-content-card munich d-flex align-items-center justify-content-center text-center bg-diagonal-gradient-primary js-tilt" data-toggle="tooltip" data-placement="top" title="2%">
                                        <div class="work-content-card-inner">
                                            <svg aria-hidden="true" class="mb-2" width="22" height="22" viewBox="0 0 18 18"><path d="M1 5c0-1.1.9-2 2-2h12a2 2 0 012 2v2H1V5z"></path><path fill="#D00" d="M1 7h16v4H1z"></path><path d="M1 11h16v2a2 2 0 01-2 2H3a2 2 0 01-2-2v-2z" fill="#FFCE00"></path></svg>
                                            <h3 class="fs-15 text-white">Munich</h3>
                                        </div>
                                    </div>
                                    {/* <!-- end work-content-card --> */}
                                </div>
                                {/* <!-- end work-content --> */}
                            </div>
                            {/* <!-- end col-lg-7 --> */}
                        </div>
                        {/* <!-- end row--> */}
                    </div>
                    {/* <!-- end container --> */}
                </section>
                {/* <!-- end career-area --> */}


                {/* 
<!-- ================================
         START CTA AREA
================================= --> */}
                <section class="cta-area pt-80px pb-50px bg-gray">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-4 responsive-column-half">
                                <div class="info-box px-3">
                                    <div class="icon-element icon-element-lg mb-4 shadow-sm rounded-rounded">
                                    <svg
                                        className="svg-icon-color-1"
                                        width={40}
                                        xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 512 512"
                                        xmlSpace="preserve"
                                    >
                                        <path d="M216.533 73.859c-42.56 16.842-70.494 58.353-70.509 103.836 0 16.512 13.358 29.983 30.004 29.983 16.619 0 30.004-13.446 30.004-29.983.04-28.447 22.936-51.607 51.606-51.628 28.136 0 51.677 23.718 51.677 51.578-.01 21.585-13.022 40.521-33.124 48.258-14.422 5.529-26.623 15.057-35.285 27.574-15.841 22.868-13.232 42.162-13.232 72.133 0 2.379.28 4.749.83 7.058a7.488 7.488 0 0 0 7.291 5.739c4.894 0 8.43-4.58 7.301-9.258-.587-2.43-.43-.715-.43-33.122.04-24.994 15.302-47.019 38.905-56.127 44.551-17.065 57.66-74.62 23.093-109.174-42.097-42.05-113.699-12.087-113.633 46.968-.02 8.248-6.741 14.977-15.002 14.996-8.261-.02-14.992-6.748-15.002-14.996-.021-54.06 45.418-100.096 102.783-96.428 49.768 3.095 90.511 44.857 90.511 96.437-.01 40.311-24.343 75.722-61.978 90.199-5.801 2.209-10.731 6.028-14.262 11.037-6.8 9.697-5.401 15.327-5.401 46.669-.02 8.248-6.751 14.976-15.002 14.987-4.14 0-7.501 3.368-7.501 7.498 0 15.746 37.47 6.154 37.505-22.485v-29.583c.019-6.297 3.87-11.715 10.041-14.124 43.609-16.751 71.639-58.213 71.599-104.196 0-78.328-79.598-132.897-152.789-103.846zM283.268 400.466c-11.691-19.365-38.662-18.963-50.564-1.052-13.07 19.676.639 46.567 25.214 46.567 22.643 0 37.73-25.007 25.35-45.515zm-29.174 30.047c-5.816-1.434-9.973-5.758-11.12-11.567-3.072-16.581 18.842-24.991 27.45-10.731 6.979 11.561-3.225 25.53-16.33 22.298z" />
                                        <path d="M384.383 34.529C251.159-42.769 79.981 14.818 20.129 156.41c-24.356 57.523-26.385 121.948-7.301 179.788 13.275 40.23 35.087 69.5 36.844 71.054l.03-.04-.02.05c.02.02.04.03.07.06.01 0 .01.011.02.021.01 0 .01.01.02.02 2.82 2.367 5.211 1.69 6.211 1.69l.02-.01c.095 0 .777-.153 1.42-.41h.02l.17-.08h.02c4.849-2.082 5.706-8.235 3.08-11.308C-55.015 237.51 60.68 15.094 256.038 15.094c134.827 0 241.029 109.953 241.029 240.943 0 133.146-107.808 240.943-241.029 240.943-51.126 0-99.982-15.817-141.277-45.729-2.3-1.67-5.47-1.879-7.971-.52L22.599 496.3c-4.003 2.141-7.616-1.354-7.591-4.429.021-1.796-1.819 2.136 31.974-60.266 2.717-5.004-.954-11.067-6.581-11.067a7.499 7.499 0 0 0-6.6 3.929C-.217 487.262-.014 485.157.007 491.871c.027 14.699 15.951 25.091 29.734 17.626l80.08-43.36c86.572 60.362 201.864 61.211 289.365 2.129 156.657-105.777 148.71-338.888-14.803-433.737z" />
                                    </svg>
                                    </div>
                                    <div class="info-body">
                                        <h3 class="fs-18 pb-4 fw-bold">Check out our FAQ about how Disilab works</h3>
                                        <a href="faq.html" class="btn theme-btn theme-btn-sm">Get help <i class="la la-arrow-right icon ml-1"></i></a>
                                    </div>
                                </div>
                                {/* <!-- end info-box --> */}
                            </div>
                            {/* <!-- end col-lg-4 --> */}
                            <div class="col-lg-4 responsive-column-half">
                                <div class="info-box px-3">
                                    <div class="icon-element icon-element-lg mb-4 shadow-sm rounded-rounded">
                                        <svg class="svg-icon-color-2" width="40" viewBox="0 0 511.765 511.765" xmlns="http://www.w3.org/2000/svg"><path d="m491.945 353.116c.15-.305.283-.617.389-.937 30.321-69.981 24.903-155.775-14.478-221.349 21.024-25.133-6.285-61.125-36.149-47.199-5.248-5.363-10.758-10.503-16.468-15.363 11.208-30.468-26.198-55.462-49.494-32.698-67.711-34.191-151.928-33.758-219.265 1.157-23.954-25.385-64.183 1.833-49.68 33.683-8.306 7.263-16.168 15.109-23.417 23.406-6.259 7.164 5.08 16.983 11.296 9.869 6.744-7.718 14.051-15.019 21.769-21.785 14.925 10.765 36.566 5.281 44.754-11.151 27.021 7.778 52.825 18.941 76.87 33.268-2.013 7.259-1.191 15.366 2.263 22.067-19.79 17.095-37.612 36.433-53.088 57.626-14.287-4.317-30.035 2.909-36.248 16.439-31.507.169-63.112 3.558-94.142 10.117-2.74-5.106-6.894-9.337-11.93-12.171 5.83-22.735 15.064-44.362 27.502-64.359 4.956-7.97-7.554-16.255-12.737-7.923-13.224 21.26-23.046 44.251-29.258 68.419-35.93.61-41.962 50.316-7.757 59.878-.504 70.742 30.082 139.52 83.12 186.372-11.064 29.887 24.855 54.539 48.559 33.459 69.178 36.785 156.607 36.79 225.789.012 24.109 23.844 62.723-2.794 49.143-33.965 27.335-24.293 48.57-54.006 62.657-86.872zm-9.45-16.774c-20.761-20.626-43.481-39.337-67.738-55.762 3.445-10.791.451-22.932-7.594-30.898 20.398-33.915 36.411-70.281 47.683-108.27 3.756.013 7.367-.68 10.694-1.934 34.539 58.333 40.856 133.543 16.955 196.864zm-345.126 84.784c-6.312-61.825 4.764-123.504 32.177-179.243 17.03 5.314 35.437-5.885 38.744-23.366 51.715 6.375 101.522 21.558 148.264 45.203-2.248 8.738-.387 18.373 4.968 25.642-52.687 66.749-122.234 116.976-201.83 145.763-4.823-7.637-12.934-12.98-22.323-13.999zm233.227-149.806c0-8.499 6.864-15.413 15.301-15.413 20.298.845 20.292 29.984 0 30.826-8.437 0-15.301-6.914-15.301-15.413zm99.617-160.32c0 8.499-6.864 15.413-15.301 15.413-20.299-.845-20.294-29.984 0-30.826 8.437 0 15.301 6.914 15.301 15.413zm-39.967-17.632c-10.366 14.565-5.555 35.62 10.073 44.277-10.919 36.644-26.399 71.722-46.092 104.441-4.117-1.184-8.687-1.478-12.92-.83-20.584-43.955-50.535-83.546-87.123-115.17 3.457-6.705 4.281-14.798 2.265-22.067 23.159-13.798 47.966-24.672 73.907-32.402 8.622 16.646 31.751 21.346 46.11 8.885 4.759 4.094 9.366 8.395 13.78 12.866zm-178.287 18.793c0-8.499 6.864-15.413 15.301-15.413 20.299.845 20.293 29.983 0 30.825-8.437 0-15.301-6.914-15.301-15.412zm145.068-70.379c8.438 0 15.302 6.914 15.302 15.413-.842 20.447-29.764 20.442-30.603 0 0-8.499 6.864-15.413 15.301-15.413zm-262.94 30.825c-8.437 0-15.301-6.914-15.301-15.413.842-20.448 29.764-20.441 30.603 0 0 8.499-6.865 15.413-15.302 15.413zm30.273-16.563c-.078-2.082-.364-4.109-.84-6.065 62.783-32.467 141.216-33.033 204.483-1.51-.813 2.719-1.258 5.594-1.273 8.571-27.286 8.072-53.377 19.474-77.735 33.961-11.708-12.111-31.757-12.115-43.469-.001-25.383-15.098-52.63-26.834-81.166-34.956zm85.905 81.282c10.142 6.922 23.845 6.923 33.988.001 34.932 30.161 63.548 67.916 83.259 109.841-1.336 1.027-2.584 2.164-3.732 3.395-49.279-24.995-101.867-40.887-156.491-47.281-1.475-4.464-3.951-8.472-7.165-11.753 14.66-19.915 31.484-38.104 50.141-54.203zm-71.738 60.095c8.437 0 15.301 6.914 15.301 15.413-.842 20.448-29.764 20.442-30.603 0 .001-8.499 6.865-15.413 15.302-15.413zm-163.668 27.218c0-8.499 6.864-15.413 15.301-15.413 20.298.845 20.292 29.983 0 30.825-8.436.001-15.301-6.913-15.301-15.412zm22.817 29.465s22.781-4.968 22.781-29.267c0 0 58.456-9.344 87.867-9.672.552 7.266 3.655 13.823 8.414 18.771-29.228 58.801-41.017 123.993-34.231 189.321-2.922 1.218-5.612 2.883-7.99 4.907-48.931-43.834-76.841-117.278-76.841-174.06zm96.411 212.665c-8.437 0-15.301-6.914-15.301-15.413s6.864-15.413 15.301-15.413c8.324 0 15.302 7.21 15.302 15.413 0 8.499-6.865 15.413-15.302 15.413zm133.173 19.877c-36.401 0-72.397-8.718-104.569-25.269 1.306-3.743 1.882-8.007 1.628-11.979 82.338-29.679 154.289-81.546 208.795-150.513 4.478 2.1 9.447 3.064 14.384 2.805 3.332 12.367 5.965 24.984 7.831 37.6 1.429 9.658 16.272 7.5 14.839-2.194-1.981-13.396-4.786-26.794-8.347-39.916 1.793-1.116 3.459-2.417 4.977-3.872 25.028 16.991 48.362 36.496 69.501 58.083-13.11 28.981-32.191 55.222-56.434 76.905-2.397-1.8-5.07-3.248-7.939-4.278 1.474-13.943 2.116-28.021 1.9-41.945-.152-9.798-15.151-9.568-14.998.232.208 13.384-.412 26.917-1.836 40.32-17.689 2.813-29.258 21.155-24.384 38.345-32.374 16.814-68.649 25.676-105.348 25.676zm134.49-18.573c-8.437 0-15.301-6.914-15.301-15.413.842-20.448 29.764-20.441 30.603 0 0 8.499-6.864 15.413-15.302 15.413z"></path></svg>
                                    </div>
                                    <div class="info-body">
                                        <h3 class="fs-18 pb-4 fw-bold">To share feedback about our platform, please visit our meta community</h3>
                                        <a href="faq.html" class="btn theme-btn theme-btn-sm">Visit Site <i class="la la-arrow-right icon ml-1"></i></a>
                                    </div>
                                </div>
                                {/* <!-- end info-box --> */}
                            </div>
                            {/* <!-- end col-lg-4 --> */}
                            <div class="col-lg-4 responsive-column-half">
                                <div class="info-box px-3">
                                    <div class="icon-element icon-element-lg mb-4 shadow-sm rounded-rounded">
                                        <svg class="svg-icon-color-3" width="40" viewBox="0 0 496 496" xmlns="http://www.w3.org/2000/svg"><path d="m496 208h-17.976562l-26.792969 14.097656c-3.015625-1.664062-6.152344-3.074218-9.535157-3.921875l-33.703124-8.433593v-3.832032c14.910156-10.40625 24.007812-27.597656 24.007812-45.910156v-13.984375c0-31.078125-24.351562-57.085937-54.28125-57.992187-1.519531-.039063-3.015625.015624-4.519531.089843l8.738281-20.457031-42.363281-42.351562-24.972657 16.253906c-9.488281-5.34375-19.523437-9.503906-29.945312-12.421875l-6.167969-29.136719h-60.96875l-6.167969 29.144531c-10.421874 2.917969-20.464843 7.078125-29.945312 12.421875l-24.972656-16.253906-42.328125 42.328125 8.589843 20.472656c-.328124-.015625-.640624-.074219-.96875-.082031-15.320312-.445312-29.75 5.144531-40.710937 15.785156-10.976563 10.632813-17.015625 24.902344-17.015625 40.183594v16c0 18.3125 9.089844 35.496094 24 45.902344v3.839844l-21.984375 5.496093-39.847656-23.238281h-26.167969v165.769531l78.703125 26.230469h15.328125l1.914062-6.703125 24.125 14.480469c.394532 13.117187 10.914063 23.671875 24.019532 24.132812.460937 13.050782 10.949218 23.539063 24 24 .460937 13.050782 10.949218 23.539063 24 24 .476562 13.355469 11.414062 24.089844 24.878906 24.089844 5 0 9.902344-1.558594 14.023438-4.335938l7.085937 6.257813c4.441406 3.910156 10.160156 6.078125 16.097656 6.078125h1.480469c13.3125 0 24.144531-10.761719 24.3125-24.03125 13.167969-.167969 23.832031-10.832031 24-24 13.175781-.167969 23.847656-10.847656 24-24.023438 13.296875-.3125 24.03125-11.175781 24.03125-24.546874 0-.804688-.039062-1.597657-.121094-2.398438l144.121094-72.054688zm-80-61.984375v13.984375c0 14.199219-7.671875 27.464844-20.023438 34.617188l-4 2.3125.03125 25.316406 39.96875 9.984375-87.976562 46.304687v-21.550781c0-10.753906-4.34375-20.601563-11.457031-27.878906l27.457031-6.859375v-25.335938l-3.992188-2.308594c-12.34375-7.136718-20.007812-20.402343-20.007812-34.601562v-16c0-10.910156 4.3125-21.113281 12.152344-28.710938 7.832031-7.59375 18.222656-11.6875 29.085937-11.265624 21.371094.640624 38.761719 19.480468 38.761719 41.992187zm-155.9375 126.230469-36.214844-11.574219c-16.503906-5.289063-34.367187-6.078125-51.261718-2.335937l-4.585938 1.015624v-2.367187c0-11.03125 7.480469-20.609375 18.175781-23.289063l45.824219-11.449218v-25.335938l-3.992188-2.308594c-12.34375-7.136718-20.007812-20.402343-20.007812-34.601562v-16c0-10.910156 4.3125-21.113281 12.152344-28.710938 7.832031-7.59375 18.289062-11.6875 29.085937-11.265624 21.371094.640624 38.761719 19.480468 38.761719 41.992187v13.984375c0 14.199219-7.671875 27.464844-20.023438 34.617188l-4 2.3125.03125 25.316406 45.816407 11.449218c10.695312 2.679688 18.175781 12.257813 18.175781 23.289063v29.976563l-1.976562 1.039062h-4.902344l-49.183594-14.054688c-3.875-1.097656-7.867188-1.546874-11.875-1.699218zm-90.269531-152.511719c15.023437-29.109375 45.261719-47.734375 78.207031-47.734375 32.632812 0 62.742188 18.375 77.878906 47.089844-3.808594 7.636718-5.878906 16.09375-5.878906 24.910156v16c0 18.3125 9.089844 35.496094 24 45.902344v3.839844l-32 8.011718-32.007812-8.011718v-3.832032c14.910156-10.40625 24.007812-27.597656 24.007812-45.910156v-13.984375c0-31.078125-24.351562-57.085937-54.28125-57.992187-15.3125-.449219-29.75 5.144531-40.710938 15.785156-10.96875 10.640625-17.007812 24.910156-17.007812 40.191406v16c0 18.3125 9.089844 35.496094 24 45.902344v3.839844l-32 8.011718-32.007812-8.011718v-3.832032c14.910156-10.40625 24.007812-27.597656 24.007812-45.910156v-13.984375c0-9.414063-2.265625-18.351563-6.207031-26.28125zm-36.792969-48.382813 25.558594-25.558593 22.496094 14.640625 4.304687-2.640625c10.785156-6.609375 22.441406-11.449219 34.640625-14.367188l4.921875-1.179687 5.566406-26.246094h35.03125l5.550781 26.238281 4.921876 1.175781c12.199218 2.921876 23.855468 7.761719 34.640624 14.371094l4.304688 2.640625 22.496094-14.640625 25.527344 25.535156-9.074219 21.246094c-6.175781 2.65625-11.902344 6.394532-16.886719 11.226563-.328125.320312-.609375.664062-.929688.992187-18.847656-29.960937-52.167968-48.785156-88.070312-48.785156-36.160156 0-69.574219 19-88.375 49.265625-5.03125-5.265625-11.015625-9.53125-17.648438-12.511719zm-29 150.894532v-25.335938l-3.992188-2.308594c-12.34375-7.136718-20.007812-20.402343-20.007812-34.601562v-16c0-10.910156 4.3125-21.113281 12.152344-28.710938 7.832031-7.59375 18.246094-11.6875 29.085937-11.265624 21.371094.640624 38.761719 19.480468 38.761719 41.992187v13.984375c0 14.199219-7.671875 27.464844-20.023438 34.617188l-4 2.3125.03125 25.316406 27.449219 6.859375c-7.113281 7.277343-11.457031 17.125-11.457031 27.878906v5.925781l-24.953125 5.546875.648437-2.265625.304688-14.785156-42.191406-24.613281zm-88-14.246094h5.832031l57.535157 33.558594-29.03125 116.121094-34.335938-11.449219zm65.296875 160-15.746094-5.246094 28.203125-112.800781 18.246094 10.640625v2.285156l-30.03125 105.121094zm54.703125 23.03125c0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625-4.945312 0-8.96875-4.023438-8.96875-8.96875zm24 24c0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625-4.945312 0-8.96875-4.023438-8.96875-8.96875zm24 24c0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625-4.945312 0-8.96875-4.023438-8.96875-8.96875zm32.96875 32.96875c-4.945312 0-8.96875-4.023438-8.96875-8.96875 0-2.359375.953125-4.671875 2.625-6.34375l14.0625-14.0625c1.671875-1.671875 3.984375-2.625 6.34375-2.625 4.945312 0 8.96875 4.023438 8.96875 8.96875 0 2.359375-.953125 4.671875-2.625 6.34375l-14.0625 14.0625c-1.671875 1.671875-3.984375 2.625-6.34375 2.625zm38.6875 8h-1.480469c-2.039062 0-3.992187-.742188-5.511719-2.089844l-5.984374-5.277344 6.007812-6.007812c2.144531-2.144531 3.808594-4.695312 5.039062-7.433594l7.273438 6.054688c1.902344 1.585937 3 3.921875 3 6.410156 0 4.601562-3.742188 8.34375-8.34375 8.34375zm71.742188-72h-1.382813c-2.007813 0-3.96875-.710938-5.503906-2l-43.382813-36.152344-10.25 12.296875 42.128906 35.101563c1.894532 1.585937 2.992188 3.921875 2.992188 6.410156 0 4.601562-3.742188 8.34375-8.34375 8.34375h-1.746094c-1.941406 0-3.839844-.6875-5.335937-1.9375l-43.453125-36.207031-10.25 12.296875 42.128906 35.105468c1.902344 1.574219 3 3.910157 3 6.398438 0 4.601562-3.742188 8.34375-8.34375 8.34375-3.070312 0-6.0625-1.089844-8.417969-3.046875l-17.855469-14.882813c-3.933593-8.039062-12.015624-13.644531-21.480468-13.980468-.460938-13.050782-10.949219-23.539063-24-24-.460938-13.050782-10.949219-23.539063-24-24-.46875-13.355469-11.40625-24.089844-24.871094-24.089844-6.574219 0-13.007812 2.664062-17.65625 7.3125l-14.0625 14.0625c-.839844.839844-1.535156 1.792969-2.242188 2.738281l-24.574218-14.746093 21.542968-75.398438 54.007813-12.007812c14.167969-3.167969 29.105469-2.480469 42.921875 1.941406l11.785156 3.769531-39.953125 19.976563c-9.128906 4.574218-14.800781 13.757812-14.800781 23.96875v1.582031c0 14.777343 12.023438 26.800781 26.800781 26.800781 4.855469 0 9.632813-1.320312 13.785157-3.824219l29.789062-17.863281c5.984375-3.585938 14.015625-2.800781 19.183594 1.847656l67.59375 60.832032c1.808594 1.640624 2.847656 3.96875 2.847656 6.40625 0 4.746093-3.855469 8.601562-8.601562 8.601562zm152.601562-92.945312-134.777344 67.394531c-.4375-.464844-.894531-.90625-1.375-1.335938l-67.582031-60.832031c-5.921875-5.34375-13.578125-8.28125-21.554687-8.28125-5.832032 0-11.558594 1.585938-16.574219 4.59375l-29.800781 17.871094c-1.671876 1.007812-3.582032 1.535156-5.535157 1.535156-5.960937 0-10.800781-4.839844-10.800781-10.800781v-1.582031c0-4.121094 2.289062-7.816407 5.96875-9.65625l44.625-22.3125c7.636719-3.824219 16.703125-4.679688 24.933594-2.3125l50.273437 14.359374 12.175781.304688 150.023438-78.960938zm0 0"></path></svg>
                                    </div>
                                    <div class="info-body">
                                        <h3 class="fs-18 pb-4 fw-bold">Looking for help with Disilab for Teams?</h3>
                                        <a href="faq.html" class="btn theme-btn theme-btn-sm">Team help <i class="la la-arrow-right icon ml-1"></i></a>
                                    </div>
                                </div>
                                {/* <!-- end info-box --> */}
                            </div>
                            {/* <!-- end col-lg-4 --> */}
                        </div>
                        {/* <!-- end row --> */}
                    </div>
                    {/* <!-- end container --> */}
                </section>
                {/* <!-- end cta-area --> */}

            </div>
            <Footer />



            {/* <!-- start back to top --> */}
            <div id="back-to-top" data-toggle="tooltip" data-placement="top" title="Return to top">
                <i class="la la-arrow-up"></i>
            </div>
            {/* <!-- end back to top --> */}

            {/* <!-- Modal --> */}
            <div class="modal fade modal-container login-form" id="loginModal" tabindex="-1" role="dialog" aria-labelledby="loginModalTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header align-items-center">
                            <h5 class="modal-title" id="loginModalTitle">Good to see you again!</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true" class="la la-times"></span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form method="post">
                                <div class="form-group">
                                    <label class="fs-14 text-black fw-medium lh-18">Email</label>
                                    <input class="form-control form--control" type="email" name="email" placeholder="Email address" />
                                </div>
                                <div class="form-group">
                                    <label class="fs-14 text-black fw-medium lh-18">Password</label>
                                    <div class="input-group mb-1">
                                        <input class="form-control form--control password-field" type="password" name="password" placeholder="Enter password" />
                                        <div class="input-group-append">
                                            <button class="btn theme-btn-outline theme-btn-outline-gray toggle-password" type="button">
                                                <svg class="eye-on" xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 0 24 24" width="22px" fill="#7f8897"><path d="M0 0h24v24H0V0z" fill="none" /><path d="M12 6c3.79 0 7.17 2.13 8.82 5.5C19.17 14.87 15.79 17 12 17s-7.17-2.13-8.82-5.5C4.83 8.13 8.21 6 12 6m0-2C7 4 2.73 7.11 1 11.5 2.73 15.89 7 19 12 19s9.27-3.11 11-7.5C21.27 7.11 17 4 12 4zm0 5c1.38 0 2.5 1.12 2.5 2.5S13.38 14 12 14s-2.5-1.12-2.5-2.5S10.62 9 12 9m0-2c-2.48 0-4.5 2.02-4.5 4.5S9.52 16 12 16s4.5-2.02 4.5-4.5S14.48 7 12 7z" /></svg>
                                                <svg class="eye-off" xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 0 24 24" width="22px" fill="#7f8897"><path d="M0 0h24v24H0V0zm0 0h24v24H0V0zm0 0h24v24H0V0zm0 0h24v24H0V0z" fill="none" /><path d="M12 6c3.79 0 7.17 2.13 8.82 5.5-.59 1.22-1.42 2.27-2.41 3.12l1.41 1.41c1.39-1.23 2.49-2.77 3.18-4.53C21.27 7.11 17 4 12 4c-1.27 0-2.49.2-3.64.57l1.65 1.65C10.66 6.09 11.32 6 12 6zm-1.07 1.14L13 9.21c.57.25 1.03.71 1.28 1.28l2.07 2.07c.08-.34.14-.7.14-1.07C16.5 9.01 14.48 7 12 7c-.37 0-.72.05-1.07.14zM2.01 3.87l2.68 2.68C3.06 7.83 1.77 9.53 1 11.5 2.73 15.89 7 19 12 19c1.52 0 2.98-.29 4.32-.82l3.42 3.42 1.41-1.41L3.42 2.45 2.01 3.87zm7.5 7.5l2.61 2.61c-.04.01-.08.02-.12.02-1.38 0-2.5-1.12-2.5-2.5 0-.05.01-.08.01-.13zm-3.4-3.4l1.75 1.75c-.23.55-.36 1.15-.36 1.78 0 2.48 2.02 4.5 4.5 4.5.63 0 1.23-.13 1.77-.36l.98.98c-.88.24-1.8.38-2.75.38-3.79 0-7.17-2.13-8.82-5.5.7-1.43 1.72-2.61 2.93-3.53z" /></svg>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group d-flex align-items-center justify-content-between">
                                    <div class="custom-control custom-checkbox fs-14">
                                        <input type="checkbox" class="custom-control-input" id="rememberMe" />
                                        <label class="custom-control-label custom--control-label" for="rememberMe">Remember me!</label>
                                    </div>
                                    <a href="#!void(0)" class="lost-pass-btn fs-14 hover-underline">Forgot Password?</a>
                                </div>
                                <div class="btn-box">
                                    <button type="submit" class="btn theme-btn w-100">
                                        Login to Account <i class="la la-arrow-right icon ml-1"></i>
                                    </button>
                                </div>
                                <p class="create-account-text text-right fs-14 pt-1">
                                    New to disilab? <a class="signup-btn text-color hover-underline" href="#!void(0)">Create account</a>
                                </p>
                                <div class="icon-element my-4 mx-auto shadow-sm fs-25">Or</div>
                                <div class="text-center">
                                    <p class="fs-15 pb-3">Login with your social network</p>
                                    <button class="btn theme-btn bg-8 mb-2 mr-2"><i class="la la-facebook mr-1"></i> Facebook</button>
                                    <button class="btn theme-btn bg-9 mb-2 mr-2"><i class="la la-twitter mr-1"></i> Twitter</button>
                                    <button class="btn theme-btn bg-10 mb-2 mr-2"><i class="la la-google mr-1"></i> Google</button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>

            {/* <!-- Modal --> */}
            <div class="modal fade modal-container signup-form" id="signUpModal" tabindex="-1" role="dialog" aria-labelledby="signUpModalTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header align-items-center">
                            <h5 class="modal-title" id="signUpModalTitle">Welcome! create your account</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true" class="la la-times"></span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form method="post">
                                <div class="form-group">
                                    <label class="fs-14 text-black fw-medium lh-18">Name</label>
                                    <input class="form-control form--control" type="text" name="text" placeholder="Your name" />
                                </div>
                                <div class="form-group">
                                    <label class="fs-14 text-black fw-medium lh-18">Email</label>
                                    <input class="form-control form--control" type="email" name="email" placeholder="Email address" />
                                </div>
                                <div class="form-group">
                                    <label class="fs-14 text-black fw-medium lh-18">Password</label>
                                    <div class="input-group mb-1">
                                        <input class="form-control form--control password-field" type="password" name="password" placeholder="Enter password" />
                                        <div class="input-group-append">
                                            <button class="btn theme-btn-outline theme-btn-outline-gray toggle-password" type="button">
                                                <svg class="eye-on" xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 0 24 24" width="22px" fill="#7f8897"><path d="M0 0h24v24H0V0z" fill="none" /><path d="M12 6c3.79 0 7.17 2.13 8.82 5.5C19.17 14.87 15.79 17 12 17s-7.17-2.13-8.82-5.5C4.83 8.13 8.21 6 12 6m0-2C7 4 2.73 7.11 1 11.5 2.73 15.89 7 19 12 19s9.27-3.11 11-7.5C21.27 7.11 17 4 12 4zm0 5c1.38 0 2.5 1.12 2.5 2.5S13.38 14 12 14s-2.5-1.12-2.5-2.5S10.62 9 12 9m0-2c-2.48 0-4.5 2.02-4.5 4.5S9.52 16 12 16s4.5-2.02 4.5-4.5S14.48 7 12 7z" /></svg>
                                                <svg class="eye-off" xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 0 24 24" width="22px" fill="#7f8897"><path d="M0 0h24v24H0V0zm0 0h24v24H0V0zm0 0h24v24H0V0zm0 0h24v24H0V0z" fill="none" /><path d="M12 6c3.79 0 7.17 2.13 8.82 5.5-.59 1.22-1.42 2.27-2.41 3.12l1.41 1.41c1.39-1.23 2.49-2.77 3.18-4.53C21.27 7.11 17 4 12 4c-1.27 0-2.49.2-3.64.57l1.65 1.65C10.66 6.09 11.32 6 12 6zm-1.07 1.14L13 9.21c.57.25 1.03.71 1.28 1.28l2.07 2.07c.08-.34.14-.7.14-1.07C16.5 9.01 14.48 7 12 7c-.37 0-.72.05-1.07.14zM2.01 3.87l2.68 2.68C3.06 7.83 1.77 9.53 1 11.5 2.73 15.89 7 19 12 19c1.52 0 2.98-.29 4.32-.82l3.42 3.42 1.41-1.41L3.42 2.45 2.01 3.87zm7.5 7.5l2.61 2.61c-.04.01-.08.02-.12.02-1.38 0-2.5-1.12-2.5-2.5 0-.05.01-.08.01-.13zm-3.4-3.4l1.75 1.75c-.23.55-.36 1.15-.36 1.78 0 2.48 2.02 4.5 4.5 4.5.63 0 1.23-.13 1.77-.36l.98.98c-.88.24-1.8.38-2.75.38-3.79 0-7.17-2.13-8.82-5.5.7-1.43 1.72-2.61 2.93-3.53z" /></svg>
                                            </button>
                                        </div>
                                    </div>
                                    <p class="fs-14 lh-20">Your password must be at least 6 characters long and must contain letters, numbers and special characters. Cannot contain whitespace.</p>
                                </div>
                                <div class="form-group">
                                    <div class="custom-control custom-checkbox fs-14">
                                        <input type="checkbox" class="custom-control-input" id="agreeCheckBox" />
                                        <label class="custom-control-label custom--control-label" for="agreeCheckBox">By signing up, you agree to our <a href="privacy-policy.html" class="text-color hover-underline">Privacy Policy.</a></label>
                                    </div>
                                </div>
                                <div class="btn-box">
                                    <button type="submit" class="btn theme-btn w-100">
                                        Register Account <i class="la la-arrow-right icon ml-1"></i>
                                    </button>
                                </div>
                                <p class="create-account-text text-right fs-14">
                                    Already on disilab? <a class="login-btn text-color hover-underline" href="#">Log in</a>
                                </p>
                                <div class="icon-element my-4 mx-auto shadow-sm fs-25">Or</div>
                                <div class="text-center">
                                    <p class="fs-15 pb-3">Create account with your social network</p>
                                    <button class="btn theme-btn bg-8 mb-2 mr-2"><i class="la la-facebook mr-1"></i> Facebook</button>
                                    <button class="btn theme-btn bg-9 mb-2 mr-2"><i class="la la-twitter mr-1"></i> Twitter</button>
                                    <button class="btn theme-btn bg-10 mb-2 mr-2"><i class="la la-google mr-1"></i> Google</button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>

            {/* <!-- Modal --> */}
            <div class="modal fade modal-container recover-form" id="recoverModal" tabindex="-1" role="dialog" aria-labelledby="recoverModalTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header align-items-center">
                            <h5 class="modal-title" id="recoverModalTitle">Reset password</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true" class="la la-times"></span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <p class="fs-15 lh-20 pb-3">
                                Enter your username or email to reset your password.
                                You will receive an email with instructions on how to reset your password. If you are experiencing problems
                                resetting your password <a href="contact.html" class="text-color hover-underline">contact us</a> or <a href="#" class="text-color hover-underline">send us an email</a>
                            </p>
                            <form method="post">
                                <div class="form-group">
                                    <label class="fs-14 text-black fw-medium lh-18">Email</label>
                                    <input class="form-control form--control" type="text" name="text" placeholder="Email address" />
                                </div>
                                <div class="btn-box">
                                    <button type="submit" class="btn theme-btn w-100">
                                        Get New Password <i class="la la-arrow-right icon ml-1"></i>
                                    </button>
                                    <p class="create-account-text text-right fs-14">
                                        Not a member? <a class="text-color signup-btn hover-underline" href="#!void(0)">Create account</a>
                                    </p>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default About